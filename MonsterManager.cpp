...
void CMonsterManager::Load(char* path) // OK
{
	...
	try
	{
		while(true)
		{
			...
			info.Life = lpMemScript->GetAsNumberLong(); //Fix Load Max Vaule
...
void CMonsterManager::SetInfo(MONSTER_INFO info) // OK
{
	...
	//=== FIx Max HP
	float CalcHPRate = (float)(gServerInfo.m_MonsterLifeRate / 100);
	
	info.Life = (__int64)(info.Life*CalcHPRate); //FIx Gia Tri	
