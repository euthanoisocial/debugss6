...
{
	this->Init();
	this->ReadGroupFile(gPath.GetFullPath("Custom\\Relife\\GrandResetGroup.txt"));
	this->ReadMainFile(gPath.GetFullPath("Custom\\Relife\\GrandResetMain.ini"));
}
...
void GRSystem::FinishResetReq(LPOBJ lpObj)
{
	...
	gCashShop.GDCashShopAddPointSaveSend(lpObj->Index, 0, this->m_GroupData[ResetGroup].RewardData[lpObj->Class].WCoinC, this->m_GroupData[ResetGroup].RewardData[lpObj->Class].WCoinP,
		this->m_GroupData[ResetGroup].RewardData[lpObj->Class].GoblinPoint, 0);
	// ----
