...
void CObjectManager::CharacterGameCloseSet(int aIndex,int type) // OK
{
	...
	lpObj->CloseCount = gServerInfo.m_ThoiGianThoatGame;
...
#endif
	else if(gBsVEvent.CheckPlayerTarget(lpObj)/* || BATTLE_SURVIVOR_BSV(lpObj->Map) != 0*/)
	{
...
#if(BOSS_GUILD == 1)
	else if (lpObj->Map == MapBossGuild)
	{
		result = gBossGuild.GetUserRespawnLocation(lpObj, &gate, &map, &x, &y, &dir, &level);
	}
#endif
...
///
		...
		//====================
#if(THUE_VIP_NEW)
		gThueVip.THUE_VIP_OPTION(lpObj, 1);
#endif
		//--
#if(DANH_HIEU_NEW)
		...
		gSachThuocTinh.SachThuocTinh_Option(lpObj,1);
#endif
		//---------
#if (RINGPEDAN_NEW)
		gRingPedan.Load_Option(lpObj, 1);
#endif
		//---------
		gHuyHieu.Option(lpObj, 1);
		//---------
		gCuongHoaWing.Option(lpObj, 1);
		//---
		gVuKhiRong.Option(lpObj, 1);
		//----
//tue add thue flag 
#if(HT_THUE_FLAG)
...
///////////////////////////////////		
	...
	if (lpObj->ArmorSetBonus != 0 && (Level15Count + Level14Count + Level13Count + Level12Count + Level11Count + Level10Count) >= 5)
	{
		if (Level15Count == 5)
		{
			...
			//5 mon +15
			lpObj->Defense += (int)((lpObj->Defense * gServerInfo.PhongThu15) / 100); // Phòng Thủ
			lpObj->AddLife += (int)((lpObj->MaxLife * gServerInfo.Mau15) / 100); // Tăng Máu
			lpObj->IgnoreDefenseRate += gServerInfo.BQPN3;	// Bỏ Qua Phòng Ngự
			lpObj->DoubleDamageRate += gServerInfo.X2Dame;	// x2 dame
		...
		{
			//5 mon +14
			lpObj->Defense += (int)((lpObj->Defense * gServerInfo.PhongThu14) / 100); // Phòng Thủ
			lpObj->AddLife += (int)((lpObj->MaxLife * gServerInfo.Mau14) / 100); // Tăng Máu
			lpObj->IgnoreDefenseRate += gServerInfo.BQPN2;	// Bỏ Qua Phòng Ngự
			lpObj->Lvtuchan = 4;
		...
		{
			//5 mon +13
			lpObj->Defense += (int)((lpObj->Defense * gServerInfo.PhongThu13) / 100); // Phòng Thủ
			lpObj->AddLife += (int)((lpObj->MaxLife * gServerInfo.Mau13) / 100); // Tăng Máu
			lpObj->IgnoreDefenseRate += gServerInfo.BQPN1;	// Bỏ Qua Phòng Ngự
			lpObj->Lvtuchan = 3;
		...
		{
			//5 mon +12
			lpObj->Defense += (int)((lpObj->Defense * gServerInfo.PhongThu12) / 100); // Phòng Thủ
			lpObj->AddLife += (int)((lpObj->MaxLife * gServerInfo.Mau12) / 100); // Tăng Máu
			lpObj->Lvtuchan = 2;
		}
		...
		{
			//5 mon +11
			lpObj->AddLife += (int)((lpObj->MaxLife * gServerInfo.Mau11) / 100); // Tăng Máu
			lpObj->Lvtuchan = 1;
		}
...
///////////////
	...
	//=====================
#if(THUE_VIP_NEW)
	gThueVip.THUE_VIP_OPTION(lpObj, 0);
#endif
	//--
#if(DANH_HIEU_NEW)
	...
	//--
#if (SACHTHUOCTINH_NEW)
	gSachThuocTinh.SachThuocTinh_Option(lpObj, 0);
#endif
	//---------
#if (RINGPEDAN_NEW)
	gRingPedan.Load_Option(lpObj, 0);
#endif
	//---------
	gHuyHieu.Option(lpObj, 0);
	//---------
	gCuongHoaWing.Option(lpObj, 0);
	/////
	gVuKhiRong.Option(lpObj, 0);
	///----
//tue add flag 
#if(HT_THUE_FLAG)
...
bool CObjectManager::CharacterInfoSet(BYTE* aRecv,int aIndex) // OK
{
	...
	lpObj->freePoints = lpMsg->freePoints;
	lpObj->GensExitTick = lpMsg->GensExitTick;
	//tue add Moc Nap///
#if(B_MOCNAP)
...
///////////////
...
#endif
#if(THUE_VIP_NEW)
	lpObj->rThueVip = lpMsg->rThueVip;
	lpObj->rThueVipTime = lpMsg->rThueVipTime;
#endif
#if(NGU_HANH_NEW)
	lpObj->rNguHanh = lpMsg->rNguHanh;
#endif
#if(NGU_HANH_NEW)
	...
	//tue add flag 
#if(HT_THUE_FLAG)
	lpObj->m_RankTitle6 = lpMsg->rDanhHieu;
	lpObj->CheckFlag = 0;
#endif
	///////////////////////////////
	...
	#if(MOCNAP == 1)
	lpObj->MOCNAPCOIN = lpMsg->MOCNAPCOIN;
...
#endif
#if(QUAEVENT == 1)
	lpObj->QUATOP1COIN = lpMsg->QUATOP1COIN;
	if (lpObj->QUATOP1COIN < 0)
	{
		lpObj->QUATOP1COIN = 0;
	}

	lpObj->QUATOP2COIN = lpMsg->QUATOP2COIN;
	if (lpObj->QUATOP2COIN < 0)
	{
		lpObj->QUATOP2COIN = 0;
	}

	lpObj->QUATOP3COIN = lpMsg->QUATOP3COIN;
...
void CObjectManager::CharacterLifeCheck(LPOBJ lpObj,LPOBJ lpTarget,int damage,int DamageType,int flag,int type,int skill,int ShieldDamage) // OK
{
	...
	if(lpTarget->Life <= 0 && lpTarget->Live != 0)
	{
		...
		{
			//--Monster Reward
			gMonsterReward.AddMonsterBonus(lpTarget,&gObj[SummonIndex]);
			...
			if (lpTarget->CreatorIndex != 0)
			{
				lpTarget->SpotKillCount++;
			}
			...
			//gNhiemVu.MonsterKill(lpObj->Index, lpTarget->Index);
			...
			// Leo thap
			if(lpTarget->Map == gServerInfo.m_LeoThapMap)
			{
				gLeoThap.MonsterDieProc(lpTarget,&gObj[SummonIndex]);
			}

            gBossGuild.MonsterDieProc(lpTarget, &gObj[SummonIndex]);
