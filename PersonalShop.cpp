...
void CPersonalShop::CGPShopItemListRecv(PMSG_PSHOP_ITEM_LIST_RECV* lpMsg,int aIndex) // OK
{
	...
	{
		//LogAdd(LOG_TEST,"CGPShopItemListRecv Debug 2");
		return;
	}
	//--
...
void CPersonalShop::CGPShopBuyItemRecv(PMSG_PSHOP_BUY_ITEM_RECV* lpMsg,int aIndex) // OK
{
	...
	if(gObjIsConnectedGP(bIndex) == 0 && gObj[bIndex].IsBot != 6)
	{
		//LogAdd(LOG_TEST,"CGPShopBuyItemRecv 2");
		return;
	}
	LPOBJ lpTarget = &gObj[bIndex];
...
void CPersonalShop::GCPShopViewportSend(int aIndex) // OK
{
	...
	{
		//if(gObjIsConnectedGP(lpObj->PShopDealerIndex) == 0 || gObj[lpObj->PShopDealerIndex].PShopOpen == 0)  // BOT_STORE
...
void CPersonalShop::GCPShopItemListSend(int aIndex,int bIndex,BYTE result,BYTE type) // OK
{
	...
	g_PersonalShopEx.SendInfoTarget(aIndex, bIndex); //Send Thong Tin Target Shop
#endif
