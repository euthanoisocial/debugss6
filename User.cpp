...
void gObjCharZeroSet(int aIndex) // OK
{
	...
	lpObj->GensExitTick = 0;
	lpObj->GensExitTickSec = 0;
...
//	lpObj->AttackCustomOfflineMoneyDelay = 0;
	...
	lpObj->CreatorIndex = -1;
	memset(lpObj->CreatorName, 0, sizeof(lpObj->CreatorName));
	lpObj->SpotTime = 0;
	lpObj->SpotX = 0;
	lpObj->SpotY = 0;
	lpObj->SpotRenewal = 0;
	lpObj->SpotKillCount = 0;
	lpObj->SpotAllowParty = 0;
	lpObj->SpotAllowGuild = 0;
//tue add Tu Chan //
#if(TU_LUYEN_NEW)
	lpObj->rTuLuyen = 0;
#endif
...
#if(THUE_VIP_NEW)
	lpObj->rThueVip = 0;
	lpObj->rThueVipTime = 0;
	lpObj->rThueVipTimeSec = 0;
#endif
#if(NGU_HANH_NEW)
	lpObj->rNguHanh = 0;
#endif
...
///////////////////////
	...
	lpObj->BProtect_X = 0;
	lpObj->BProtect_Y = 0;
	lpObj->BProtectMove_Time = 0;
	lpObj->BProtectAnimationTime = 0;
	lpObj->BPCashShopTime = 0;
	lpObj->IsAutoMoveRunning = 0;
	ZeroMemory(lpObj->bp_LastAttackTime, sizeof(lpObj->bp_LastAttackTime));
...
void gObjUserDie(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	// Leo thap
	if(lpObj->Map == gServerInfo.m_LeoThapMap)
	{
		gLeoThap.UserDieProc(lpObj,lpTarget);
		return;
	}
	...
	gPKSystem.UserDieProc(lpObj, lpTarget);
...
void gObjPlayerKiller(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	#if(BOSS_GUILD == 1)
	if (gBossGuild.CheckPlayerTarget(lpObj, lpTarget, 0) != 0)
	{
		return;
	}
#endif
	...
	if (gPKSystem.CheckSafeZone(lpObj) != 0)
	{
		return;
	}
	...
	for(n = 0; n < MAX_OBJECT;n++)
	{
		...
		if(lpObj->Connected > OBJECT_LOGGED)
		{
			...
			if(lpObj->Type == OBJECT_USER)
			{
				...
				//==========================================
#if(THUE_VIP_NEW)
				if (lpObj->rThueVipTime > 0 && lpObj->rThueVip > 0)
				{
					lpObj->rThueVipTimeSec++;

					if (lpObj->rThueVipTimeSec > 60)
					{
						lpObj->rThueVipTimeSec = 0;
						lpObj->rThueVipTime--;

						if (lpObj->rThueVipTime < 1)
						{
							lpObj->rThueVipTime = 0;
							lpObj->rThueVip = 0;
						}
...
#endif
				//==========================================
				if(lpObj->GensExitTick > 0)
				{
					lpObj->GensExitTickSec++;

					if(lpObj->GensExitTickSec > 60)
					{
						lpObj->GensExitTickSec = 0;
						lpObj->GensExitTick--;

						if(lpObj->GensExitTick < 1)
						{
							lpObj->GensExitTick = 0;
							gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(867));
						}
					}
				}
...
bool gObjRebuildMasterSkillTree(LPOBJ lpObj)
{
	...
	if (lpObj->Coin1 < gServerInfo.CommandResetMaster_WCoinC)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(2025), gServerInfo.CommandResetMaster_WCoinC);
		return 0;
	}
	...
	gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, gServerInfo.CommandResetMaster_WCoinC, 0, 0, 0);
