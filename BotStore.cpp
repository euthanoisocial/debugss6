...
void ObjBotStore::AddItem(int bIndex,int botNum)
{
	...
	{ 
		...
		//LogAdd(LOG_RED,"BotStore AddItem Debug 1");
		if(blank != 255)
		{
			...
			//gItemManager.InventoryAddItem(bIndex,item,blank);
			//LogAdd(LOG_RED,"BotStore AddItem Debug 2 %d" ,blank);
		}
	}	
}
		...
		for(int botNum=0;botNum<MAX_BOTSTORE;botNum++)
		{
			if(this->bot[botNum].Enabled == true)
			{
				...
				if(result >= 0)
				{
					...
					//gObjSetMonster(result, this->bot[botNum].Class);
