...
bool CNpcTalk::NpcTalk(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
   ...
   if(gChienTruongCo.Dialog(lpObj->Index,lpNpc->Index) != 0)
	...
	if (gGrandResetSystem.Dialog(lpObj, lpNpc))
	{
		return 1;
	}
	...
	{
		GCChatTargetSend(lpObj,lpNpc->Index,gMessage.GetMessage(872));
		return 1;
	}

	// Leo thap
	if (gLeoThap.Dialog(lpObj,lpNpc))
	{
		return 1;
	}

	#if(BOSS_GUILD == 1)
	if (gBossGuild.Dialog(lpObj->Index, lpNpc->Index) != 0)
	{
		return 1;
	}
#endif
	if (gTiecRuouEvent.Dialog(lpObj,lpNpc))
    {
		return 1;
	}
switch(lpNpc->Class)
	{
		...
		case 380:
			#if(SKY_EVENT == 1)
			    if(SkyEvent.NPCEnabled == 1 && lpObj->Map == MAP_LORENCIA){ //<< Chỏ Map em có thể tùy thích cho nó ở map nào thì sửa lại map đó
...
void CNpcTalk::NpcWarehouse(LPOBJ lpNpc,LPOBJ lpObj) // OK
{
	...
	if (gServerInfo.m_CustomWarehouseSwitch == 0 || gServerInfo.m_CustomWarehouseEnable[lpObj->AccountLevel] == 0)
	{
		lpObj->Interface.use = 1;
...
/*void CNpcTalk::CGNpcTalkCloseRecv(int aIndex) // OK
{
	...
	switch(lpObj->Interface.type)
	{
		...
		case INTERFACE_WAREHOUSE:
			gWarehouse.CGWarehouseClose(aIndex,0);
			break;
