...
//************ Packet Base *********************//
//**********************************************//
struct PBMSG_HEADKY	// Packet - Byte Type
{
...
//-----------------------------------
struct PMSG_UPDATE_MENU_RECV
{
	PSBMSG_HEAD header;
	int MENU_NUMBER;
};
void UPDATEMENU(PMSG_UPDATE_MENU_RECV* lpMsg, int aIndex);
...
struct PMSG_TIECRUOU_GC
{
	PBMSG_HEAD2 h;	//đoạn này thay theo nguồn của bạn, PBMSG_HEAD3 hoặc PBMSG_HEAD2 tuỳ theo
	DWORD m_dwEventTimeTiecRuou;
};
void TiecRuouSendGC(LPOBJ lpObj);

#if(DANGKYTK)
struct PMSG_PARTY_MAIN_RECV
{ 
	PSBMSG_HEAD header; // C3:F1:01
	char account[10];
	char password[10]; 
...
void CGRegistroMainRecv(PMSG_PARTY_MAIN_RECV* lpMsg, int aIndex);
#endif
...
/////abcd
struct PMSG_UPD_CAPNHATMU_INFO
{
	PSBMSG_HEAD	h;
	int CAPNHAT1;
	int CAPNHAT2;
...
void SendCustomCAPNHATMu(LPOBJ lpObj);//ok	
#if(THANMA)
