...
void CBloodCastle::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if (gServerInfo.m_BCVIP == 1)
	{
		if (lpObj->Map == MAP_BLOOD_CASTLE8){
			lpTarget->ScoreKill++;
			gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(1005), lpTarget->ScoreKill, gServerInfo.m_DiemBlood);
			if (lpTarget->ScoreKill == gServerInfo.m_DiemBlood)
			{
			...
			this->NoticeSendToAll(&this->m_BloodCastleLevel[level], 0, gMessage.GetMessage(1006), lpTarget->Name, lpTarget->ScoreKill);
			return;
			}
			if (lpTarget->ScoreKill > gServerInfo.m_DiemBlood)
			{
