...
void JGConnectAccountRecv(SDHP_CONNECT_ACCOUNT_RECV* lpMsg) // OK
{
	...
	//antiflood
	gIpManager.InsertRealUser(gObj[lpMsg->index].IpAddr);
...
#if(DANGKYTK)
void GJRegistroMainSend(char* account, char* password, char* name , char* phone, char* numcode,char*email) // OK
{
	SDHP_REGISTRO_MAIN_SEND pMsg;

	pMsg.header.set(0x31, sizeof(pMsg));

	memcpy(pMsg.account, account, sizeof(pMsg.account) - 1);

	memcpy(pMsg.password, password, sizeof(pMsg.password) - 1);

	memcpy(pMsg.name, name, sizeof(pMsg.name) - 1);

	memcpy(pMsg.phone, phone, sizeof(pMsg.phone) - 1);

	memcpy(pMsg.numcode, numcode, sizeof(pMsg.numcode) - 1);

	memcpy(pMsg.email, email, sizeof(pMsg.email) - 1);
