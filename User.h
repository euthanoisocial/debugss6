...
#define PET_INVENTORY_BINARY_SIZE	1224
...
struct QUATOP1_USER_DATA
{
	int		QUATOP1;
};

struct QUATOP2_USER_DATA
{
	int		QUATOP2;
};

struct QUATOP3_USER_DATA
{
	int		QUATOP3;
};
#endif
///abcd
struct CAPNHAT_USER_DATA
{
	int CAPNHAT1;
	int CAPNHAT2;
	int CAPNHAT3;
...
struct OBJECTSTRUCT
{
	...
	struct GUILD_INFO_STRUCT* GuildUnion;
	char GuildName[11];
	int GuildStatus;
...
//----------
	...
	BYTE	pPetInventory[PET_INVENTORY_BINARY_SIZE];

#if (SACHTHUOCTINH_NEW)
	SACHTHUOCTINH_USER_DATA SACHTHUOCTINH;
#endif

#if (RINGPEDAN_NEW)
	RINGPEDAN_USER_DATA RINGPEDAN;
#endif
...
//tue add Tu chan //
	...
	///////////////////
#if (MOCNAP == 1)
	MOCNAP_USER_DATA QUANAP;
	int MOCNAPCOIN;
#endif
#if (QUAEVENT == 1)
	QUATOP1_USER_DATA QUATOPEVENT1;
	int QUATOP1COIN;

	QUATOP2_USER_DATA QUATOPEVENT2;
	int QUATOP2COIN;

	QUATOP3_USER_DATA QUATOPEVENT3;
	int QUATOP3COIN;
#endif
...
//MC bot
	...
	int m_ManaShieldMaxRate;			// MANA_SHIELD 
...
int orderskill;
	...
	DWORD BProtectAnimationTime;
	DWORD BPCashShopTime;
	//===Antihack 2022
	float GroupSkill[256];
	DWORD bp_LastAttackTime[6];
	DWORD bp_DetectCount;
	//===
