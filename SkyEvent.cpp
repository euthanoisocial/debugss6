...
void cSkyEvent::SetState_BLANK(){
	LogAdd(LOG_BLACK,"[EventSinhTu­] SetState_BLANK");
}
...
void cSkyEvent::SetState_EMPTY(){
	...
	LogAdd(LOG_BLACK,"[EventSinhTu­] SetState_EMPTY");
}
...
void cSkyEvent::CheckSync(){
	...
	LogAdd(LOG_BLACK,"[EventSinhTu­] Sync Start Time. [%d] min remain",this->m_RemainTime/60);
}
...
int cSkyEvent::MonsterDie(LPOBJ lpObj){
	...
	if(iLevel > -1){
		...
		if(iUser > -1){
			...
			LogAdd(LOG_EVENT,"error-L3 : [EventSinhTu­] (%d) doesn't match the user (%d) %s",
...
void cSkyEvent::ReadMonsters(char * FilePath){
	...
	if(this->Enabled == 1){
		...
		if (FilePath == NULL || !strcmp(FilePath, "")){
			ErrorMessageBox("[EventSinhTu] file load error - File Name Error");
			return;
		}
		SMDFile = fopen(FilePath, "r");
		if (!SMDFile){
			ErrorMessageBox("[EventSinhTu] file load error - fopen In: %s",FilePath);
			return;
		}
					...
					for(int i = 0; i < this->Count; i++){
						...
						if (info.Level <= MAX_SKY_EVENT_LEVEL && iStages <= MAX_SKY_EVENT_STAGES && MobCount < MAX_SKY_EVENT_STAGE_MOB_COUNT){
							...
							if(MobPos >= 0){
								...
								LogAdd(LOG_EVENT,"[EventSinhTu] - Cant add monster to Lvl:%d, Stage:%d, Count:%d [ID:%d:,X:%d,Y:%d] Pos:%d",
								 info.Level,iStages,MobCount,this->MobID,info.X,info.Y,MobPos);
							}
						}else{
							LogAdd(LOG_EVENT,"[EventSinhTu][Overflow] - Cant add monster to Lvl:%d, Stage:%d, Count:%d [ID:%d:,X:%d,Y:%d]",
		...
		fclose(SMDFile);
		LogAdd(LOG_RED,"[EventSinhTu] Monster Loaded: +(%d)",this->MonsterLoaded++);
	}
#endif
}
...
void cSkyEvent__InsideTrigger(void * lpParam){
	...
	for(int i = 5; i > 0; i--) {
		LogAdd(LOG_EVENT, "[EventSinhTu] will close after %d minute(s)", i);
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(1012),i);
		Sleep(60000);
	}
	SkyEvent.NPCEnabled = 0;	
	LogAdd(LOG_EVENT,"[EventSinhTu­] Event entrance is closed!");
	...
	LogAdd(LOG_EVENT,"[EventSinhTu­] SetState: END");
				...
				if (this->m_UserData[iLevel][iUser].DBNumber == gObj[m_Index].DBNumber){
					if((gObj[this->m_UserData[iLevel][iUser].m_Index].Map == MAP_ICARUS) && (gObj[this->m_UserData[iLevel][iUser].m_Index].X >= 130)) {
						if(this->CurrentStage == 1){
							gNotice.NewNoticeSend(this->m_UserData[iLevel][iUser].m_Index,0,0,0,0,0,"[EventSinhTử] - Kinh Nghiệm: %d",this->ExtraExp[iLevel][0]);
						}
						if(this->CurrentStage == 2){
							gNotice.NewNoticeSend(this->m_UserData[iLevel][iUser].m_Index,0,0,0,0,0,"[EventSinhTử] - Kinh Nghiệm: %d",this->ExtraExp[iLevel][1]);
						}
						if(this->CurrentStage == 3){
							gNotice.NewNoticeSend(this->m_UserData[iLevel][iUser].m_Index,0,0,0,0,0,"[EventSinhTử] - Kinh Nghiệm: %d",this->ExtraExp[iLevel][2]);
						}
...
int cSkyEvent::CheckEnterLevel(int iIndex){
	...
	for(int iLevel = 0; iLevel < MAX_SKY_EVENT_LEVEL; iLevel++){
		if(pLevel >= Level_Min[iLevel] && pLevel <= Level_Max[iLevel])
			return iLevel;
		LogAdd(LOG_BLACK,"[EventSinhTử­] Level %d",iLevel+1);
	}
	return -1;
}
...
void cSkyEvent::AddUser(BYTE iLevel,int iIndex,LPOBJ lpObj){
	...
	LogAdd(LOG_BLACK,"[EventSinhTu­] [Level %d] - So nguoi tham gia %d",iLevel+1,this->PlayerCount[iLevel]);
}
#endif
