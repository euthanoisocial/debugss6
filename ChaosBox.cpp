...
void CChaosBox::PlusItemLevelMix(LPOBJ lpObj,int type) // OK
{
	...
	else
	{
		...
		gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(1557), lpObj->Name, gItemManager.GetItemName(item.m_Index));
...
void CChaosBox::Wing2Mix(LPOBJ lpObj,int type) // OK
{
	...
	if((GetLargeRand()%100) < lpObj->ChaosSuccessRate)
	{
		if(type == 0)
		{
			...
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(1560),lpObj->Name,gItemManager.GetItemName(ItemIndex));
	...
	else
	{
		...
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(1562),lpObj->Name);
...
void CChaosBox::Wing1Mix(LPOBJ lpObj) // OK
{
	...
	else
	{
		...
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(1561),lpObj->Name);
...
void CChaosBox::Wing3Mix(LPOBJ lpObj) // OK
{
	...
	else
	{
		...
		gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(1563),lpObj->Name);
...
void CChaosBox::CustomItemMix(LPOBJ lpObj, int HeadCode) // OK
{
	...
	if(CustomMix != 0)
	{
		...
		//-- primera comprobaci�n de item bag no cambiantes
		...
		//validaci�n de item por grupos existentes
		for(int Group = 0 ; Group < CustomMix->m_CountGroup ; Group++)
		{
			...
			for(int slot = 0;slot < CHAOS_BOX_SIZE;slot++)
			{
				...
				if(gCustomMix.istItemMix(CustomMix->m_Index, Group, lpObj->ChaosBox[slot]))	// validaci�n si existe el item
				{
					itemMixCount++;
				}
			}
		...
		if((itemMixCount + ItemBagMixCount) != CustomMix->m_CountItem) // fall� la combinaci�n
		{
			this->GCChaosMixSend(lpObj->Index, 7, 0);
			return;
		}
		...
		if((GetLargeRand()%100) < lpObj->ChaosSuccessRate)
		{
			...
			GDCreateItemSend(lpObj->Index,
				...
				gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(1558),lpObj->Name,gItemManager.GetItemName(ItemIndex));
		...
		{
			CItem item = lpObj->ChaosBox[ItemIndex];
			...
			gNotice.GCNoticeSendToAll(0,0,0,0,0,0,gMessage.GetMessage(1559),lpObj->Name,gItemManager.GetItemName(item.m_Index));
