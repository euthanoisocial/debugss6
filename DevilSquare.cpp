...
void CDevilSquare::MonsterDieProc(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if (gServerInfo.m_DVVIP == 1)
	{
		if (lpObj->Map == MAP_DEVIL_SQUARE2)
		{
			lpTarget->ScoreKillDevil++;
			gNotice.GCNoticeSend(lpTarget->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(1005), lpTarget->ScoreKillDevil, gServerInfo.m_DiemDevil);
			if (lpTarget->ScoreKillDevil == gServerInfo.m_DiemDevil)
			{
				...
				this->NoticeSendToAll(&this->m_DevilSquareLevel[level], 0, gMessage.GetMessage(1007), lpTarget->Name, lpTarget->ScoreKillDevil);
				return;
			}
			if (lpTarget->ScoreKillDevil > gServerInfo.m_DiemDevil)
			{
