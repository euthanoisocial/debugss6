...
void ProtocolCore(BYTE head,BYTE* lpMsg,int size,int aIndex,int encrypt,int serial) // OK
{
	...
	if (gObj[aIndex].Type == OBJECT_USER &&lpMsg[0] == 0xC1 && head == 0xFB)
	{
		...
		case 0x60:
			gParty.CGNewLeader(aIndex, (GC_PARTYNEWLEADER*)lpMsg);
			break;
#endif
	...
	if (gBProtect.BProtectPacketRecv(head, lpMsg, size, aIndex, encrypt, serial) == false) 
	{		
	    return;
	}
	...
	case 0x4E:
		...
		case 0xBF:
			switch(lpMsg[3])
			{
				...
				case 0x70:
					...
					break;
				case 0xFF:
					SwitchServer.SwitchServerReq((ECCG_SWITCHSERVER_REQ*)lpMsg, aIndex);
					break;
				default:
		...
		case 0xF3:
			...
			switch(lpMsg[3])
			{
...
#if(DANGKYTK)
				case 0xEF: //add
					CGRegistroMainRecv((PMSG_PARTY_MAIN_RECV*)lpMsg, aIndex);
					break; 
#endif
				case 0xF0:
				...
				case 0x37: //Info Auto Move Running
					...
					break;
				case 0x4A: //Info Auto tele
					GCAutoTele((AUTOTELE_REQ*)lpMsg, aIndex);
				break;
			...
			{
				case 0x02:
					UPDATEMENU((PMSG_UPDATE_MENU_RECV*)lpMsg, aIndex);
					break;
			}
			break;
		...
		case 0xFC:
			...
			case 0x01:
				gBUY_TINH_NANG.BUY_TINH_NANG_GAME(lpObj, (BUY_TINH_NANG_REQ*)lpMsg);
				break;
#endif
#if(THUE_VIP_NEW)
			case 0x02:
				gThueVip.BUY_THUE_VIP(lpObj, (BUY_THUE_VIP_REQ*)lpMsg);
				break;
#endif
...
#if(NGU_HANH_NEW)
			case 0x07:
				gNguHanh.BUY_NGU_HANH(lpObj, (BUY_NGU_HANH_REQ*)lpMsg);
				break;
#endif
			...
			case 0x09:
				gSachThuocTinh.BUG_SACHTHUOCTINH(lpObj, (SACHTHUOCTINH_REQ*)lpMsg);
				break;
#endif
				//--------------------------------------
#if(RINGPEDAN_NEW)
			case 0x10:
				gRingPedan.BUG_RINGPEDAN(lpObj, (BUY_RINGPEDAN_REQ*)lpMsg);
				break;
#endif
				//--------------------------------------
#if(MOVE_EVENT_NEW)
			case 0x11:
				gMOVE_EVENT.MOVE_EVENT_GAME(lpObj, (MOVE_EVENT_REQ*)lpMsg);
				break;
#endif
				//--------------------------------------
...
#endif
			//tue add Bom Mau
		case 103: gAUTOHP.CommandHp(lpObj); (&gObj[aIndex], "", 0);	
			break;
			//-----
			//tue add Tự Buff
		case 104: gAddBuffer.CommandAddBuff(&gObj[aIndex]);	
			break;
			//------------
			}
			break;
		...
		case 0xFD:
			...
			break;	
		case 0xFE:
			g_NewsBoard.OpenItem(&gObj[aIndex], (NEWS_REQ_NEWS*)lpMsg);
			break;
		...
		case 0xFF:
			...
			case 0x02:
				gGrandResetSystem.SendResetData(&gObj[aIndex], true);
				break;
			case 0x03:
				gGrandResetSystem.FinishResetReq(&gObj[aIndex]);
				break;
			case 0x06:
			...
			case 0x09:
				...
				break;
			case 0x90:
				gLeoThap.RecvLeoThap((PMSG_LEOTHAP_RECV*)lpMsg, aIndex);
				break;
#if(DOIMK)
								//==== Change Pass
			case 0x91:
...
void GCAutoTele(AUTOTELE_REQ* lpMsg, int aIndex) {
	LPOBJ lpObj = &gObj[aIndex];
	...
	gObjTeleport(lpObj->Index,lpMsg->Map,lpMsg->MapX,lpMsg->MapY);

	//lpObj->IsAutoMoveRunning = lpMsg->TYPE;
	LogAdd(LOG_RED, "Check Tele  Map %d | X %d | Y %d",lpMsg->Map,lpMsg->MapX,lpMsg->MapY );
...
void CGMoveRecv(PMSG_MOVE_RECV* lpMsg,int aIndex) // OK
{
	...
	for(int n=0;n < MAX_VIEWPORT;n++)
	{
		if(lpObj->VpPlayer2[n].type == OBJECT_USER)
		{
			...
			if (gEffectManager.CheckStunEffect(lpObj) != 0 || gEffectManager.CheckImmobilizeEffect(lpObj) != 0)
		{
			//===BProtect Anti Hack Khang Stun,Sleep...
			lpObj->LastMoveTime = GetTickCount();
			//=======================================
			return;
		}
	...
	//{
	//	//===BProtect MOVE
	//	gBProtect.KiemTraDiChuyen(aIndex, lpObj->TX, lpObj->TY);
	//}
}
...
void ChatSend(LPOBJ lpObj, char* szChat)//ok2
{
...
void GCNewCharacterInfoSend(LPOBJ lpObj) // OK
{
	...
	pMsg.ViewGReset = (DWORD)(lpObj->MasterReset);
...
void GCNewHealthBarSend(LPOBJ lpObj) // OK
{
	...
	for (int n = 0; n < MAX_VIEWPORT; n++)
	{
		...
		CItem* VK1 = &lpTarget->Inventory[0];
		CItem* VK2 = &lpTarget->Inventory[1];
		CItem* Helm = &lpTarget->Inventory[2];
		CItem* Armor = &lpTarget->Inventory[3];
		CItem* Pants = &lpTarget->Inventory[4];
		CItem* Gloves = &lpTarget->Inventory[5];
		CItem* Boots = &lpTarget->Inventory[6];
		CItem* Wing = &lpTarget->Inventory[7];
		...
		info.Reset = lpTarget->Reset;

		//----
		info.ItemID = VK1->m_Index;
		info.ItemID = VK2->m_Index;
		info.ItemID = Helm->m_Index;
		info.ItemID = Armor->m_Index;
		info.ItemID = Pants->m_Index;
		info.ItemID = Gloves->m_Index;
		info.ItemID = Boots->m_Index;
		info.ItemID = Wing->m_Index;
		//-----
		...
		if (lpTarget->Type == OBJECT_USER)
		{
			...
			if (gVuKhiRong.m_VuKhiRong.Enabled)
			{
			BYTE bHas = 0;
			for (size_t i = 0; i < 6; i++)
			{
			if (lpTarget->Inventory[i].IsItem() &&
				lpTarget->Inventory[i].m_Index == GET_ITEM(0,19)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(0,51)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(0,56)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(0,105)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(0,106)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(0,110)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(2,13)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(2,25)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(2,31)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(4,18)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(4,30)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(4,33)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(5,10)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(5,49)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(5,65)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(5,36)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(5,50)
				||lpTarget->Inventory[i].m_Index == GET_ITEM(5,66)
				&& lpTarget->Inventory[i].m_Level >= 13)
...
{
	LogAdd(LOG_RED, "[Req] Number = %d", lpMsg->Number);
	LPOBJ lpObj = &gObj[aIndex];
	...
	LogAdd(LOG_RED, "Get %d %d %d %d %d", lpObj->Interface.use, lpObj->Interface.type, lpObj->Interface.state, lpObj->LoadWarehouse, lpObj->WarehouseLock);
}

void CGWarehouseOpen(PMSG_WAREHOUSEINTERFACE_RECV* lpMsg, int aIndex)
{
	if (gServerInfo.m_CustomWarehouseSwitch == 0)
	{
		return;
	}
	...
	if (gServerInfo.m_CustomWarehouseEnable[lpObj->AccountLevel] == 0)
	{
	...
	gLog.Output(LOG_COMMAND, "[CommandWare][%s][%s] - (Number: %d)", lpObj->Account, lpObj->Name, lpMsg->Number);
...
{
	int totalStats = lpMsg->Str + lpMsg->Agi + lpMsg->Vit + lpMsg->Ene + lpMsg->Com;
	if (gObj[aIndex].LevelUpPoint < totalStats)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "[StatsAdd] An error has occurred. Contact Administrator!");
		return;
	}

	if (lpMsg->Str < 0)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "[StatsAdd] Error. Strength less than zero.");
		return;
	}
	if (lpMsg->Agi < 0)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "[StatsAdd] Error. Agility less than zero.");
		return;
	}
	if (lpMsg->Vit < 0)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "[StatsAdd] Error. Vitality less than zero.");
		return;
	}
	if (lpMsg->Ene < 0)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "[StatsAdd] Error. Energy less than zero.");
		return;
	}
	if (lpMsg->Com < 0)
	{
		gNotice.GCNoticeSend(aIndex, 1, 0, 0, 0, 0, 0, "[StatsAdd] Error. Command less than zero.");
		return;
	}
...
//=====================================================================
#if(NGU_HANH_NEW)
void GetInfoNguHanh(LPOBJ lpObj)
{
	PMSG_NGUHANH_UPD pMsg;
	pMsg.h.set(0xFB, 0x38, sizeof(pMsg)); // Gửi Lên Client Protocol.cpp
	// ----
	pMsg.rNguHanh = lpObj->rNguHanh;
...
//=====================================================================
//////////chuyensub
void UPDATEMENU(PMSG_UPDATE_MENU_RECV* lpMsg, int aIndex)//gui tu client ve
{
	LPOBJ lpObj = &gObj[aIndex];
	switch (lpMsg->MENU_NUMBER)
		{
		case 0:			
			break;
			//--------------------------------------------
			case 1:
			gCommandManager.CommandSelectSub(lpObj,"1");
			break;
		case 2:
			gCommandManager.CommandSelectSub(lpObj,"2");
			break;
		case 3:
			gCommandManager.CommandSelectSub(lpObj,"3");
			break;
		case 4:
			gCommandManager.CommandSelectSub(lpObj,"4");
			break;
		case 5:
			gCommandManager.CommandSelectSub(lpObj,"5");
			break;
		case 6:
			gCommandManager.CommandSelectSub(lpObj,"6");
			break;
			//--------------------------------------------------
...
//=====================================================================
#if(RESETLIFE_NEW)
void GetInfoTinhNang(LPOBJ lpObj)
{
	// ----
	PMSG_TINHNANG_UPD pMsg;
	pMsg.h.set(0xFB, 0x55, sizeof(pMsg)); // Gửi Lên Client Protocol.cpp

	pMsg.rResetLife = lpObj->rResetLife;
	pMsg.CommandResetPoint_WCoinC = gServerInfo.CommandResetPoint_WCoinC;
	pMsg.CommandResetMaster_WCoinC = gServerInfo.CommandResetMaster_WCoinC;
	pMsg.CommandChange_WCoinC = gServerInfo.CommandChange_WCoinC;
	pMsg.CommandRelife_Check_Level = gServerInfo.CommandRelife_Check_Level;
	pMsg.CommandRelife_Check_Reset = gServerInfo.CommandRelife_Check_Reset;
	pMsg.CommandRelife_Point = gServerInfo.CommandRelife_Point;
	pMsg.CommandRelife_MaxClient = gServerInfo.CommandRelife_MaxClient;
...
//=====================================================================
#if(SACHTHUOCTINH_NEW)
void GetInfoThuocTinh(LPOBJ lpObj)
{
	// ----
	PMSG_THUOCTINH_UPD pMsg;
	pMsg.h.set(0xFB, 0x56, sizeof(pMsg)); // Gửi Lên Client Protocol.cpp
	// ----
	//pMsg.SachThuocTinh_01 = lpObj->SACHTHUOCTINH.SACHTHUOCTINH_01;
	pMsg.SachThuocTinh_02 = lpObj->SACHTHUOCTINH.SACHTHUOCTINH_02;
	pMsg.SachThuocTinh_03 = lpObj->SACHTHUOCTINH.SACHTHUOCTINH_03;
	pMsg.SachThuocTinh_04 = lpObj->SACHTHUOCTINH.SACHTHUOCTINH_04;
	pMsg.SachThuocTinh_05 = lpObj->SACHTHUOCTINH.SACHTHUOCTINH_05;
	pMsg.SachThuocTinh_06 = lpObj->SACHTHUOCTINH.SACHTHUOCTINH_06;
	pMsg.SachThuocTinh_07 = lpObj->SACHTHUOCTINH.SACHTHUOCTINH_07;
	// ----
	DataSend(lpObj->Index, (BYTE*)&pMsg, sizeof(pMsg));
}

void GetInfoThuocTinhAtive(LPOBJ lpObj)
{
	// ----
	PMSG_THUOCTINH_ATIVE_UPD pMsg;
	pMsg.h.set(0xFB, 0x57, sizeof(pMsg)); // Gửi Lên Client Protocol.cpp
	// ----
	pMsg.OPTION_LEVEL_01 = gSachThuocTinh.OPTION_LEVEL_01;
	pMsg.OPTION_LEVEL_02 = gSachThuocTinh.OPTION_LEVEL_02;
	pMsg.OPTION_LEVEL_03 = gSachThuocTinh.OPTION_LEVEL_03;
	pMsg.OPTION_LEVEL_04 = gSachThuocTinh.OPTION_LEVEL_04;
	pMsg.OPTION_LEVEL_05 = gSachThuocTinh.OPTION_LEVEL_05;
	pMsg.OPTION_LEVEL_06 = gSachThuocTinh.OPTION_LEVEL_06;
	pMsg.OPTION_LEVEL_07 = gSachThuocTinh.OPTION_LEVEL_07;
	pMsg.OPTION_LEVEL_08 = gSachThuocTinh.OPTION_LEVEL_08;
	pMsg.OPTION_LEVEL_09 = gSachThuocTinh.OPTION_LEVEL_09;
	pMsg.OPTION_LEVEL_10 = gSachThuocTinh.OPTION_LEVEL_10;
	pMsg.OPTION_LEVEL_11 = gSachThuocTinh.OPTION_LEVEL_11;
	pMsg.OPTION_LEVEL_12 = gSachThuocTinh.OPTION_LEVEL_12;
	pMsg.OPTION_ACTIVE_01 = gSachThuocTinh.OPTION_ACTIVE_01;
	pMsg.OPTION_ACTIVE_02 = gSachThuocTinh.OPTION_ACTIVE_02;
	pMsg.OPTION_ACTIVE_03 = gSachThuocTinh.OPTION_ACTIVE_03;
	pMsg.OPTION_ACTIVE_04 = gSachThuocTinh.OPTION_ACTIVE_04;
	pMsg.OPTION_ACTIVE_05 = gSachThuocTinh.OPTION_ACTIVE_05;
	pMsg.OPTION_ACTIVE_06 = gSachThuocTinh.OPTION_ACTIVE_06;
	pMsg.OPTION_ACTIVE_07 = gSachThuocTinh.OPTION_ACTIVE_07;
	pMsg.OPTION_ACTIVE_08 = gSachThuocTinh.OPTION_ACTIVE_08;
	pMsg.OPTION_ACTIVE_09 = gSachThuocTinh.OPTION_ACTIVE_09;
	pMsg.OPTION_ACTIVE_10 = gSachThuocTinh.OPTION_ACTIVE_10;
	pMsg.OPTION_ACTIVE_11 = gSachThuocTinh.OPTION_ACTIVE_11;
	pMsg.OPTION_ACTIVE_12 = gSachThuocTinh.OPTION_ACTIVE_12;
...
//=====================================================================
#if(RINGPEDAN_NEW)
void GetInfoRingPedan(LPOBJ lpObj)
{
	// ----
	PMSG_RINGPEDAN_UPD pMsg;
	pMsg.h.set(0xFB, 0x58, sizeof(pMsg)); // Gửi Lên Client Protocol.cpp
	pMsg.SachThuocTinh_01 = lpObj->SACHTHUOCTINH.SACHTHUOCTINH_01;
	pMsg.RingPedan1 = lpObj->RINGPEDAN.RINGPEDAN1; //lpObj->RANKTITLE.RankTitle1; //lpObj->rDanhHieu;
	pMsg.RingPedan2 = lpObj->RINGPEDAN.RINGPEDAN2;
	pMsg.RingPedan3 = lpObj->RINGPEDAN.RINGPEDAN3;
	pMsg.RingPedan4 = lpObj->RINGPEDAN.RINGPEDAN4;

	pMsg.AutoResetEnable = lpObj->AutoResetEnable;
	pMsg.AttackCustom = lpObj->AttackCustom;
	pMsg.GHRS = gCustomGHRS.maxResets;
...
void RPSModeInface(int aIndex)
{
	...
	pMsg.header.set(0xFB, 0x88, sizeof(pMsg));
	DataSend(aIndex, (BYTE*)&pMsg, pMsg.header.size);
}
#endif
//-----------------------------------
	...
	void GCCharacterPageSend(LPOBJ lpObj)
	{
		...
		pMsg.header.set(0xF3, 0xD4, sizeof(pMsg));
...
void TiecRuouSendGC(LPOBJ lpObj)
{
	// ----
	PMSG_TIECRUOU_GC pMsg;
	pMsg.h.set((LPBYTE)&pMsg, 0xFB, 0xC2, sizeof(pMsg));

	pMsg.m_dwEventTimeTiecRuou	= gTiecRuouEvent.m_CustomTiecRuouInfo[0].RemainTime;

	DataSend(lpObj->Index, (LPBYTE)&pMsg, sizeof(pMsg));
}
#if(HUY_HIEU_CUONG_HOA)
void GetInfoHuyHieu(LPOBJ lpObj)
{
	// ----
	PMSG_HUYHIEU_UPD pMsg;
	pMsg.h.set(0xFB, 0x89, sizeof(pMsg)); // Gửi Lên Client Protocol.cpp

	pMsg.LvHHCH = lpObj->Lvtuchan;
...
void CGRegistroMainRecv(PMSG_PARTY_MAIN_RECV* lpMsg, int aIndex) // OK
{
	LogAdd(LOG_RED, "LLEGO PROTOCOLCORE 2");

	char account[10] = { 0 };
	memcpy(account, lpMsg->account, (sizeof(account)  ));

	char password[10] = { 0 };
	memcpy(password, lpMsg->password, (sizeof(password) ));

	char name[50] = { 0 };
	memcpy(name, lpMsg->name, (sizeof(name) ));

	char phone[30] = { 0 };
	memcpy(phone, lpMsg->phone, (sizeof(phone) ));

	char numcode[18] = { 0 };
	memcpy(numcode, lpMsg->numcode, (sizeof(numcode)));

	char email[50] = { 0 };
	memcpy(email, lpMsg->email, (sizeof(email)));

	GJRegistroMainSend(account, password,name, phone, numcode,email);
}
#endif
...
#endif
////abcd
void SendCustomCAPNHATMu(LPOBJ lpObj)
{
	PMSG_UPD_CAPNHATMU_INFO pMsg;
	pMsg.h.set(0xF4, 0xF6, sizeof(pMsg));
	// ----
	pMsg.CAPNHAT1 = lpObj->AccountLevel;
	pMsg.CAPNHAT2 = lpObj->Reset;
	pMsg.CAPNHAT3 = lpObj->Map;
	pMsg.CAPNHAT4 = lpObj->MasterLevel;
	pMsg.CAPNHAT5 = lpObj->X;
	pMsg.CAPNHAT6 = lpObj->Y;
	pMsg.CAPNHAT7 = lpObj->DBClass;
	pMsg.CAPNHAT8 = lpObj->Level;
	
	DataSend(lpObj->Index, (BYTE*)&pMsg, sizeof(pMsg));//ok
}	
//===================================================>