...
void cChangeClass::ChangeClass(LPOBJ lpObj, int Class)
{
	...
	lpObj->Level = 400;
	lpObj->Experience = 0;
	lpObj->MasterLevel = lpObj->MasterLevel; //Done
	lpObj->MasterExperience = 0;
	lpObj->MasterPoint = lpObj->MasterPoint;
	for (int n = 0; n < MAX_SKILL_LIST; n++) //Clear SKill
	{
		lpObj->Skill[n].Clear();
	}

	for (int n = 0; n < MAX_MASTER_SKILL_LIST; n++) //Clear Master SKill
	{
