...
bool CAttack::Attack(LPOBJ lpObj,LPOBJ lpTarget,CSkill* lpSkill,bool send,BYTE flag,int damage,int count,bool combo) // OK
{
	...
	if (!gMonsterCall.CheckAttack(lpObj, lpTarget))
	{
		return 0;
	}
	...
	#if(BOSS_GUILD == 1)
	if (gBossGuild.CheckPlayerTarget(lpObj, lpTarget, 1))
	{
		return 0;
	}
#endif
	...
	if(lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_MONSTER)
	{
		//LogAdd(LOG_RED,"CHECK 0");

		if(combo == 0 && skill != SKILL_NONE)
		{		
			if(lpTarget->SummonIndex > 1 )
			{
				//gLog.Output(LOG_HACK, "[HackAutoSkill][%s][%s] (%d)", lpObj->Account, lpObj->Name, skill);
				return 0;
			}
	...
	if(damage == 0)
	{
		...
		if (skill == SKILL_FIRE_SLASH || skill == SKILL_POWER_SLASH || skill == SKILL_BLAST || skill == SKILL_CYCLONE || skill == SKILL_SLASH || skill == SKILL_TWISTING_SLASH || skill == SKILL_SWORD_SLASH || skill == SKILL_LIGHTNING_STORM || skill == SKILL_CRESCENT_MOON_SLASH || skill == SKILL_STAR_FALL || skill == SKILL_IMPALE || skill == SKILL_FIRE_BREATH || skill == SKILL_ICE_ARROW || skill == SKILL_PENETRATION || skill == SKILL_SPIRAL_SLASH || skill == SKILL_FROZEN_STAB || skill == SKILL_CHARGE || skill == SKILL_BLOOD_STORM)
		{
			if (skill != SKILL_IMPALE || lpObj->Inventory[8].m_Index == GET_ITEM(13, 2) || lpObj->Inventory[8].m_Index == GET_ITEM(13, 3) || lpObj->Inventory[8].m_Index == GET_ITEM(13, 37))
			{
	...
	if(lpObj->Type == OBJECT_USER)
	{
		...
		else
		{
			...
			//LogAdd(LOG_RED,"Debug Dame %d", damage);
		}
	}

	#pragma endregion
	////==============================MoGioiHanDameQuai===========================>
	//	if (gServerInfo.m_GeneralDamageRatePvM > gServerInfo.m_MoGioiHanDameQuai)
	//		{
	//			float gCalcDameRate = (float)(gServerInfo.m_GeneralDamageRatePvM / gServerInfo.m_MoGioiHanDameQuai);
	//			damage = damage*gCalcDameRate;
	//		}
	//		else {
	//			damage = (damage*gServerInfo.m_GeneralDamageRatePvM) / gServerInfo.m_MoGioiHanDameQuai;
	//		}

	//		if (gServerInfo.m_DamageRatePvM[lpObj->Class] > gServerInfo.m_MoGioiHanDameQuai)
	//		{
	//			float CalcDameRate = (float)(gServerInfo.m_DamageRatePvM[lpObj->Class] / gServerInfo.m_MoGioiHanDameQuai);
	//			damage = damage*CalcDameRate;
	//		}
	//		else {
	//			damage = (damage*gServerInfo.m_DamageRatePvM[lpObj->Class]) / gServerInfo.m_MoGioiHanDameQuai;
	//		}
	...
	if(lpObj->Type == OBJECT_USER && effect != 4)
	{
		...
		for (std::vector<CUSTOM_START_ITEM_DAME_INFO>::iterator it = lpInfo->ItemDameStart.begin(); it != lpInfo->ItemDameStart.end(); it++)
		{
			...
			{
				damage = (damage *(100 + it->DameRate)) / 100;
			}
	...
	if(damage > 0)
	{
		if(lpObj->Type == OBJECT_USER && lpTarget->Type == OBJECT_USER && lpObj->Index != lpTarget->Index)
		{
			lpObj->m_SecondReflect =+ damage;
			gServerInfo.GCDamageTable(lpObj->Index);
		}
		else
		{
			lpObj->m_SecondDamage = damage;
			lpTarget->m_SecondDefence =+ damage;
			gServerInfo.GCDamageTable(lpObj->Index);
			gServerInfo.GCDamageTable(lpTarget->Index);
		}
...
void CAttack::DamageSprite(LPOBJ lpObj,int damage) // OK
{
	...
	{
		lpItem->m_Durability -= (damage*(1.0f*DurabilityValue))/500000;
	}
#endif
	else
	{
...
bool CAttack::CheckPlayerTarget(LPOBJ lpObj,LPOBJ lpTarget) // OK
{
	...
	if(lpObj->Guild != 0 && lpTarget->Guild != 0)
	{
			...
			if(lpObj->Guild->Number == lpTarget->Guild->Number && gServerInfo.m_CungGuildPK != 0 && gDuel.CheckDuel(lpObj,lpTarget) == 0 
	...
	if(gMapManager.GetMapGensBattle(lpObj->Map) != 0 && gMapManager.GetMapGensBattle(lpTarget->Map) != 0)
	{
		...
		// Leo thap
	if(!gLeoThap.CheckPlayerTarget(lpObj) && lpObj->Map == gServerInfo.m_LeoThapMap)
	{
		return 0;
	}
...
void CAttack::GetPreviewDamageMultiplier(LPOBJ lpObj,DWORD* DamageMultiplier,DWORD* RFDamageMultiplierA,DWORD* RFDamageMultiplierB,DWORD* RFDamageMultiplierC) // OK
{
	switch(lpObj->Class)
	{
		...
		case CLASS_FE:
			...
			break;
		case CLASS_MG:
			(*DamageMultiplier) = lpObj->MGDamageMultiplierRate;
			(*RFDamageMultiplierA) = 100;
			(*RFDamageMultiplierB) = 100;
			(*RFDamageMultiplierC) = 100;
			break;
...
#if(ANTIHACKGS)
	if(lpObj->Type == OBJECT_USER) {
		if(!gBProtect.AntiDelayAttack(aIndex)) {
			return;
		}
	}
#endif
