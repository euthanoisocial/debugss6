...
CServerDisplayer::CServerDisplayer() // OK
{
	...
	this->m_font5 = CreateFont(14, 0, 0, 0, FW_DONTCARE, 0, 0, 0, VIETNAMESE_CHARSET, OUT_DEFAULT_PRECIS, CLIP_DEFAULT_PRECIS, DEFAULT_QUALITY, DEFAULT_PITCH | FF_DONTCARE, "MS Sans Serif");
...
void CServerDisplayer::SetWindowName() // OK
{
	...
	if(ISPREMIUN)
	{
		wsprintf(buff,"[%s] %s (Online Players: %d) %s",VERSION,gServerInfo.m_ServerName, gObjTotalUser,GAMESERVER_CLIENT);
	}
	else
	{																													  
		wsprintf(buff,"[%s] %s (ON: %d) %s   (License Free)",GAMESERVER_VERSION,gServerInfo.m_ServerName, gObjTotalUser,GAMESERVER_CLIENT);
	}
...
void CServerDisplayer::PaintName() // OK nombre
{
	...
	FillRect(hdc,&rect,this->m_brush[2]);
		...
		{
			DrawText(hdc, GAMESERVER_VERSION, sizeof(GAMESERVER_VERSION), &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
		}
#else
		{
			DrawText(hdc, GAMESERVER_VERSION, sizeof(GAMESERVER_VERSION), &rect, DT_CENTER | DT_VCENTER | DT_SINGLELINE);
...
void CServerDisplayer::PaintEventTime() // OK
{
	...
	//--
	SetTextColor(hdc,RGB(0,102,204));
	wsprintf(text1, "Sinh Tu: ");

	if (this->EventSinhTu == -1)
	{
		wsprintf(text2, "Disabled");
	}
	else if (this->EventSinhTu == 0)
	{
		wsprintf(text2, "Online");
	}
	else
	{
		totalseconds	= this->EventSinhTu;
		hours			= totalseconds/3600;
		minutes			= (totalseconds/60) % 60;  
		seconds			= totalseconds % 60;
		wsprintf(text2, "%02d:%02d:%02d", hours, minutes, seconds);
	...
	TextOut(hdc, posX1, rect.top + 2 + 210, text1, strlen(text1));
	if (this->BEventThanMa == -1)
	{
		SetTextColor(hdc, RGB(255, 0, 0));
	}
	...
	TextOut(hdc, posX2, rect.top + 2 + 210, text2, strlen(text2));
#endif
	//============================================================================
	SetTextColor(hdc, RGB(0, 102, 204));
	wsprintf(text1, "Event BossGuild: ");

	if (this->EventBossGuild == -1)
	{
		wsprintf(text2, "Disabled");
	}
	else if (this->EventBossGuild == 0)
	{
		wsprintf(text2, "Online");
	}
	else
	{
		totalseconds = this->EventBossGuild;
		hours = totalseconds / 3600;
		minutes = (totalseconds / 60) % 60;
		seconds = totalseconds % 60;
		wsprintf(text2, "%02d:%02d:%02d", hours, minutes, seconds);
	...
	else if (this->EventBossGuild < 300)
	{
		SetTextColor(hdc, RGB(0, 190, 0));
	}
	else
...
void CServerDisplayer::PaintInvasionTime() // OK
{
	...
	SetTextColor(hdc,RGB(0, 173, 181));
	TextOut(hdc,rect.left+5,rect.top+2,"EventBoss:",9);

	for (int n=0; n < 22; n++)
	{

		SetTextColor(hdc,RGB(0,102,204));
		if (n==0) { wsprintf(text1,"%d.%s",n, "Hoang Kim: "); }
		if (n==1) { wsprintf(text1,"%d.%s",n, "Tho Ngoc: "); }
		if (n==2) { wsprintf(text1,"%d.%s",n, "Medusa: "); }
		if (n==3) { wsprintf(text1,"%d.%s",n, "Selupan: "); }
		if (n==4) { wsprintf(text1,"%d.%s",n, "Kundun: "); }
		if (n==5) { wsprintf(text1,"%d.%s",n, "Rong Do: "); }
		if (n==6) { wsprintf(text1,"%d.%s",n, "Rong 2 Dau: "); }
		if (n==7) { wsprintf(text1,"%d.%s",n, "Quy Vuong: "); }
		if (n==8) { wsprintf(text1,"%d.%s",n, "Boss Class: "); }
		if (n==9) { wsprintf(text1,"%d.%s",n, "Boss The Gioi: "); }
		if (n==10){ wsprintf(text1,"%d.%s",n, "Kho Bau HT: "); }
		if (n==11){ wsprintf(text1,"%d.%s",n, "Boss Bang Hoi: "); }
		if (n==12){ wsprintf(text1,"%d.%s",n, "Boss Guild: "); }
		if (n==13){ wsprintf(text1,"%d.%s",n, "CT Sinh Tu: "); }
		if (n==14){ wsprintf(text1,"%d.%s",n, "Dau TruongPK­: "); }
		if (n==15){ wsprintf(text1,"%d.%s",n, "Leo Thap: "); }
		if (n==16){ wsprintf(text1,"%d.%s",n, "ChienTruongCo: "); }
		if (n==17){ wsprintf(text1,"%d.%s",n, "Chua Co: "); }
		if (n==18){ wsprintf(text1,"%d.%s",n, "Chua Co: "); }
		if (n==19){ wsprintf(text1,"%d.%s",n, "Chua Co: "); }
		if (n==20){ wsprintf(text1,"%d.%s",n, "Chua Co: "); }
		if (n==21){ wsprintf(text1,"%d.%s",n, "Chua Co: "); }
		if (n==22){ wsprintf(text1,"%d.%s",n, "Chua Co: "); }
