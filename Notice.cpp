...
void CNotice::GCNoticeSendToAllKanturu(BYTE type,BYTE count,BYTE opacity,WORD delay,DWORD color,BYTE speed,char* message,...) // OK
{
	char buff[256] = {0};
	...
	for(int n = OBJECT_START_USER ; n < MAX_OBJECT ; n++)
	{
		...
		if(gObjIsConnectedGP(n) != 0 
			&& (lpObj->Map == MAP_KANTURU1
			|| lpObj->Map == MAP_KANTURU2
			|| lpObj->Map == MAP_KANTURU3))
		{
			DataSend(n,(BYTE*)&pMsg,pMsg.header.size);
		}
	}
