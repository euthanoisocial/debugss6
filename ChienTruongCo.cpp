...
void cChienTruongCo::CGPacketCTC(XULY_CGPACKET* aRecv, int aIndex)
{
	...
	{
		gObjMoveGate(lpObj->Index, 822); //sua gate nay
	...
	else if(ThaoTac == 2)
	{
		...
		{
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(964),this->MaxBuyDay);
			return;
		}
		if(lpObj->Coin1 < this->WCoinBuy) {
			gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(965),this->WCoinBuy);
			return;
		}
		GDSetCoinSend(lpObj->Index, -(this->WCoinBuy), 0, 0,gMessage.GetMessage(966));
		lpObj->CTCTime += (this->TimeBuyMin*60);
		lpObj->CTCRegDay++;
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(967),this->TimeBuyMin);
