 ...
 int CItemBag::GetItemNewOption_New(ITEM_BAG_INFO* lpInfo,int Slot) // OK
{
	...
	while(true)
	{
		...
		{
			RandomManager.AddElement(1,gServerInfo.NewItemDrop[0]);
			RandomManager.AddElement(2,gServerInfo.NewItemDrop[1]);
			RandomManager.AddElement(4,gServerInfo.NewItemDrop[2]);
			RandomManager.AddElement(8,gServerInfo.NewItemDrop[3]);
			RandomManager.AddElement(16,gServerInfo.NewItemDrop[4]);
			RandomManager.AddElement(32,gServerInfo.NewItemDrop[5]);
		}  
		else if (Slot >= 0 &&  Slot <=  1) 			 //Vu Khi
		{
 			RandomManager.AddElement(1,gServerInfo.NewVKDrop[0]);
			RandomManager.AddElement(2,gServerInfo.NewVKDrop[1]);
			RandomManager.AddElement(4,gServerInfo.NewVKDrop[2]);
			RandomManager.AddElement(8,gServerInfo.NewVKDrop[3]);
			RandomManager.AddElement(16,gServerInfo.NewVKDrop[4]);
			RandomManager.AddElement(32,gServerInfo.NewVKDrop[5]);
		}
		else if (Slot >= 9 &&  Slot <=  10) 	  //Day Chuyen Ring
		{
 			RandomManager.AddElement(1,gServerInfo.NewPhuKienDrop[0]);
			RandomManager.AddElement(2,gServerInfo.NewPhuKienDrop[1]);
			RandomManager.AddElement(4,gServerInfo.NewPhuKienDrop[2]);
			RandomManager.AddElement(8,gServerInfo.NewPhuKienDrop[3]);
			RandomManager.AddElement(16,gServerInfo.NewPhuKienDrop[4]);
			RandomManager.AddElement(32,gServerInfo.NewPhuKienDrop[5]);
		}
		//====
		...
		if((NewOption & value) == 0)
		{
			NewOption |= value;
			count++;
			//LogAdd(LOG_RED,"GetItemNewOption1 %d ~ %d (Type %d)",NewOption,count,Slot);
		}
	}
...
bool CItemBag::GetItem(LPOBJ lpObj,CItem* lpItem) // OK
{
	...
	int Option3 = lpInfo->Option3;
		//=== Phan Loai Itme Drop
	int NewOption	= 0;  
	//LogAdd(LOG_RED,"DEBUG 	m_CheckOptionsDrop %d, %d",gServerInfo.m_CheckOptionsDrop,gItemManager.GetItemSlotByIndex(lpInfo->Index));
	   	...
	   	if(gServerInfo.m_CheckOptionsDrop) 
	{
		...
		{
			Option3 = gServerInfo.m_MaxItemOption;
		}
		else
		{
			Option3 = 1;
