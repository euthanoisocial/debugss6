...
bool CCommandManager::ManagementCore(LPOBJ lpObj,char* message, int Npc) // OK
{
	...
	switch(code)
	{
        ...
        case COMMAND_MARRY:
            ...
            break;
		case COMMAND_SELECT:
            Result = CommandSelectSub(lpObj,argument);
            break;
		...
		case COMMAND_DROPALL:
            ...
            break;
		case COMMAND_STARTBSV:
			Result = CommandStartBsV(lpObj,argument);
			break;
			#if(MOCNAP == 1)
...
#endif
		#if(QUAEVENT == 1)
		case COMMAND_SETQUATOP1:
			Result = CommandSetQuaTop1(lpObj, argument);
			break;
		case COMMAND_SETQUATOP2:
			Result = CommandSetQuaTop2(lpObj, argument);
			break;
		case COMMAND_SETQUATOP3:
			Result = CommandSetQuaTop3(lpObj, argument);
			break;
#endif
        //tue add bom mau ///////
		case COMMAND_AUTOHP:			Result = gAUTOHP.CommandHp(lpObj);										
			break;
		//tue add Tự Buff
		case COMMAND_ADDBUFF:			Result = gAddBuffer.CommandAddBuff(lpObj);								
			break;
		// ------
		// ////////////////////
		default:
...
bool CCommandManager::CommandChange(LPOBJ lpObj,char* arg,int Npc) // OK
{
	...
	if (lpObj->Coin1 < gServerInfo.CommandChange_WCoinC)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(2026), gServerInfo.CommandChange_WCoinC);
		return 0;
	}
	...
	gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, gServerInfo.CommandChange_WCoinC, 0, 0, 0);
...
bool CCommandManager::CommandReset(LPOBJ lpObj,char* arg,int Npc) // OK
{
	...
	if (lpObj->Reset >= gCustomGHRS.maxResets) {
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(1000), lpObj->Reset, gCustomGHRS.maxResets);
		return 0;
	}
#endif
...
// cusstom
	...
	// NEW
#if(POINT_SAU_KHI_RESET)
	lpObj->LevelUpPoint += gServerInfo.m_CommandResetPoint[lpObj->AccountLevel];
#endif
	...
	gNotice.GCNoticeSendToAll(0, 0, 0, 0, 0, 0, gMessage.GetMessage(2023), lpObj->Name, lpObj->Reset);
...
bool CCommandManager::CommandChangeClass(LPOBJ lpObj,char* arg,int Npc) // OK
{
	...
	if(lpObj->Interface.use != 0)
	{
		...
		lpObj->MasterLevel = lpObj->MasterLevel;  //0
		lpObj->MasterPoint = lpObj->MasterPoint;  //0
...
bool CCommandManager::CommandReAdd(LPOBJ lpObj,char* arg,int Npc)
{
	...
	if (lpObj->Coin1 < gServerInfo.CommandResetPoint_WCoinC)
	{
		gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, gMessage.GetMessage(2024), gServerInfo.CommandResetPoint_WCoinC);
		return 0;
	}
	...
	gCashShop.GDCashShopSubPointSaveSend(lpObj->Index, 0, gServerInfo.CommandResetPoint_WCoinC, 0, 0, 0);
...
bool CCommandManager::CommandSet(LPOBJ lpObj,char* arg) // OK
{
	...
	if(lpTarget == 0)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(900));
		return 0;
	}
	...
	for(int n=0;n < socket && socket <= MAX_SOCKET_OPTION;n++)
	{
		...
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0, " hihi ");
...
bool CCommandManager::CommandSelectSub(LPOBJ lpObj,char* arg) // OK
{
	...
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,gMessage.GetMessage(954));
		return 0;
	}
	#if (GAMESERVER_CLIENTE_UPDATE >=13)

	int sub = (this->GetNumber(arg,0)-1);

	if(gServerInfo.m_CommandSelectSub[lpObj->Account] == 0)
       
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"[Select Sub] Bạn không phải VIP !");
		return 0;
	}
	...
	GJMapServerMoveSend(lpObj->Index, sub, (BYTE)lpObj->Map,(BYTE)lpObj->X, (BYTE)lpObj->Y);

	gLog.Output(LOG_COMMAND,"[CommandSelectSub][%s][%s] - Used",lpObj->Account,lpObj->Name);
    #endif
	return 1;
}
...
#if(QUAEVENT == 1)
bool CCommandManager::CommandSetQuaTop1(LPOBJ lpObj, char* arg) // OK
{
	char name[11] = { 0 };
	...
	lpTarget->QUATOP1COIN += value;
	//gMocNap.GDSaveUserInfo(lpTarget->Index);
	GDCharacterInfoSaveSend(lpTarget->Index);
	
	LogAdd(LOG_EVENT, "[SetDiemQuaTop1][%s][%s] + (DIEM QUA TOP1: %d)", lpObj->Account, lpObj->Name, value);
...
#if(QUAEVENT == 1)
bool CCommandManager::CommandSetQuaTop2(LPOBJ lpObj, char* arg) // OK
{
	char name[11] = { 0 };
	...
	lpTarget->QUATOP2COIN += value;
	//gMocNap.GDSaveUserInfo(lpTarget->Index);
	GDCharacterInfoSaveSend(lpTarget->Index);
	
	LogAdd(LOG_EVENT, "[SetDiemQuaTop2][%s][%s] + (DIEM QUA TOP2: %d)", lpObj->Account, lpObj->Name, value);
...
#if(QUAEVENT == 1)
bool CCommandManager::CommandSetQuaTop3(LPOBJ lpObj, char* arg) // OK
{
	char name[11] = { 0 };
	...
	lpTarget->QUATOP3COIN += value;
	//gMocNap.GDSaveUserInfo(lpTarget->Index);
	GDCharacterInfoSaveSend(lpTarget->Index);
	
	LogAdd(LOG_EVENT, "[SetDiemQuaTop3][%s][%s] + (DIEM QUA TOP3: %d)", lpObj->Account, lpObj->Name, value);
