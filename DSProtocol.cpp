...
void DataServerProtocolCore(BYTE head,BYTE* lpMsg,int size) // OK
{
	...
	switch(head)
	{
		...
		case 0xB0:
			...
			case 0x04: //-> Answer Hornor Data
				gSachThuocTinh.DGGetData((SACHTHUOCTINH_DG_GET_DATA*)lpMsg);
				break;
#endif

#if(BOSSKILL_NEW)
			case 0x06: //-> Answer Hornor Data
				gBossKill.DGGetData((BOSSKILL_DG_GET_DATA*)lpMsg);
				break;
#endif
#if(RINGPEDAN_NEW)
			case 0x07: //-> Answer Hornor Data
				gRingPedan.DGGetData((RINGPEDAN_DG_GET_DATA*)lpMsg);
				break;
#endif
...
#if(MOCNAP == 1)
			case 0x09: //-> 
				gMocNap.DGGetData((MOCNAP_DG_GET_DATA*)lpMsg);
				break;
#endif
#if(QUAEVENT == 1)
			case 0x10: //-> 
				gQuaTop1.DGGetData((QUATOP1_DG_GET_DATA*)lpMsg);
				break;
			case 0x11: //-> 
				gQuaTop2.DGGetData((QUATOP2_DG_GET_DATA*)lpMsg);
				break;
			case 0x12: //-> 
				gQuaTop3.DGGetData((QUATOP3_DG_GET_DATA*)lpMsg);
				break;
#endif
			}break;
			//====================================================================================
...
#endif
#if (BOTSTATUE == 1)
			case 0xFE: //Info Tao Tuong Nhan vat
			{
				ObjBotStatue.BotGetInfoSet((SDHP_BOT_INFO_RECV*)lpMsg);
			}
			break;
#endif
#if (GHRS_TOP1_NEW)
...
void DGCharacterInfoRecv(SDHP_CHARACTER_INFO_RECV* lpMsg) // OK
{
	...
	if((lpObj->Map == 98)) { //Move về lorencia khi thoát game vào lại, nếu thay đổi thì cần đồng map 98
	...
	///////////////////
#if(SKY_EVENT)
	if(lpObj->Map == MAP_ICARUS){//đoạn này xử lý khi nhật vật thoát game sẽ về devias
		gObjMoveGate(lpObj->Index, 22);
	}
#endif
#if(THANMA)
		if (lpObj->Map == gThanMaChien.EventMap) {//đoạn này xử lý khi nhật vật thoát game sẽ về devias
		gObjMoveGate(lpObj->Index, 17);
	}
#endif
#if (SACHTHUOCTINH_NEW)
	gSachThuocTinh.GDReqData(lpObj->Index);
	GetInfoThuocTinh(lpObj);
	GetInfoThuocTinhAtive(lpObj);
#endif
#if(RINGPEDAN_NEW)
	gRingPedan.GDReqData(lpObj->Index);
	//GetInfoRingPedan(lpObj);
#endif
...
#endif
#if(MOCNAP == 1)
	gMocNap.GDReqData(lpObj->Index);
	MocNapDataSend(lpObj->Index);
#endif
	//===Send Client
#if(QUAEVENT == 1)
	gQuaTop1.GDReqData(lpObj->Index);
	QuaTop1DataSend(lpObj->Index);

	gQuaTop2.GDReqData(lpObj->Index);
	QuaTop2DataSend(lpObj->Index);

	gQuaTop3.GDReqData(lpObj->Index);
	QuaTop3DataSend(lpObj->Index);
#endif	
	//===========
	...
	g_PersonalShopEx.NPCDialog(lpObj);
	...
	{
		if (lpItem->m_Index == GET_ITEM(it->ItemType, it->ItemIndex))
		{
			gEffectManager.AddEffect(lpObj, 0, it->EffectBuff, (int)(time(0) + it->TimeBuff), it->Power1Buff, it->Power2Buff, 0, 0);

#if(BOT_NV_NEW)
			int index;
			for (int i = 1; i <= it->Power2Buff; i++)
			{
				index = gObjAddSummon();
				...
				gObjSetMonster(index, it->Power1Buff);

				lpSummon->PhysiDamageMin += 1000000;
				lpSummon->PhysiDamageMax += 1000000;
				lpSummon->CurseDamageMin += 1000000;
				lpSummon->CurseDamageMax += 1000000;

				lpSummon->SummonIndex = lpObj->Index;
				lpSummon->Attribute = 100;
				lpSummon->TargetNumber = -1;
				lpSummon->AttackType = -1;
				lpSummon->ActionState.Attack = 1;
				...
				//gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "Tạo Boss");
		...
		else
		{
			...
			for (int n = MAX_OBJECT_MONSTER; n < OBJECT_START_USER; n++)
			{
				if (gObj[n].SummonIndex == lpObj->Index)
				{
					...
					gObjDel(n);
					//gNotice.GCNoticeSend(lpObj->Index, 1, 0, 0, 0, 0, 0, "XÓA BOSS");
				}
			}
#endif
	...
	gGrandResetSystem.SendResetData(lpObj, false);

	gChangeClass.SendData(lpObj->Index);

	gServerInfo.SendServerInfo(lpObj->Index);
	...
	if (info != 0 && lpObj->Socket == INVALID_SOCKET)
	{
		...
		} /*else if(info->TuDongReset == 2 && lpObj->AutoResetVipEnable == 0) {
...
#endif
	///////////////////////////////
	if (gServerInfo.NewsWindowOnLogin == TRUE)
	{
...
void GDCharacterInfoSaveSend(int aIndex) // OK
{
	...
	pMsg.freePoints = lpObj->freePoints;
	pMsg.GensExitTick = lpObj->GensExitTick;
//tue add Moc nap ///////////
#if(B_MOCNAP)
...
#endif
#if(THUE_VIP_NEW)
	pMsg.rThueVip = lpObj->rThueVip;
	pMsg.rThueVipTime = lpObj->rThueVipTime;
#endif
#if(NGU_HANH_NEW)
...
//tue add thue flag
#if(HT_THUE_FLAG)
	pMsg.m_RankTitle6 = lpObj->rDanhHieu;
	pMsg.CheckFlag = 0;
#endif
///////////////////////////////
	//==CTC Save ve DB
	...
	#if(MOCNAP == 1)
	pMsg.MOCNAPCOIN = lpObj->MOCNAPCOIN;
#endif
	#if(QUAEVENT == 1)
	pMsg.QUATOP1COIN = lpObj->QUATOP1COIN;
	pMsg.QUATOP2COIN = lpObj->QUATOP2COIN;
	pMsg.QUATOP3COIN = lpObj->QUATOP3COIN;
#endif
...
void GDBotInfoSend(int aIndex,int Rank,int TypeTop) // OK
{
	...
	pMsg.header.set(0xFE,sizeof(pMsg));

	pMsg.index = aIndex;

	pMsg.Rank = Rank;

	pMsg.TypeTop = TypeTop;

	gDataServerConnection.DataSend((BYTE*)&pMsg,pMsg.header.size);

	LogAdd(LOG_RED,"DEBUG GDBotInfoSend >> Lay Du Lieu Nhan Vat Top %d Type %d",Rank,TypeTop);
}
