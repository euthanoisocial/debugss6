...
bool CSkillManager::CheckSkillTarget(LPOBJ lpObj,int aIndex,int bIndex,int type) // OK
{
	...
	//LogAdd(LOG_RED,"%d %d %d %d %d",lpObj->Name,aIndex,bIndex,type,OBJECT_RANGE(gObj[aIndex].SummonIndex) );
...
int CSkillManager::AddSkill(LPOBJ lpObj,int index,int level) // OK
{
	...
	/*if(index == SKILL_ENERGY_BALL){
	if(lpObj->SummonIndex > 1 ) {return 0;}}*/
...
void CSkillManager::UseAttackSkill(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	if(lpObj->Type == OBJECT_USER && lpObj->Class == CLASS_DW)
	{
		...
		if(lpObj->Teleport != 0 && gBCustomEdit.CheckSkillOK(skill)) {
				return;
		}
...
bool CSkillManager::SkillManaShield(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	if(BATTLE_SURVIVOR_BSV(lpObj->Map) != 0 && aIndex != bIndex)
	{
		return 0;
	}
...
bool CSkillManager::SkillDWTEST(int aIndex,int bIndex,CSkill* lpSkill,BYTE x,BYTE y,bool combo) // OK
{
	LPOBJ lpObj = &gObj[aIndex];
...
bool CSkillManager::SkillSummonParty(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	if(lpObj->Map == 94 || lpObj->Map == MapCTC || lpObj->Map == gServerInfo.m_LeoThapMap)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"Trong Map Nay Ban Khong The Dung Skill Nay");
		return 0;
	}


	if(lpPObj->Map == 94 || lpPObj->Map == MapCTC || lpObj->Map == gServerInfo.m_LeoThapMap)
	{
		gNotice.GCNoticeSend(lpObj->Index,1,0,0,0,0,0,"Trong Map San Ngoc Ban Khong The Dung Skill Nay");
		return 0;
	}
...
bool CSkillManager::SkillChainDriver(int aIndex,int bIndex,CSkill* lpSkill,bool combo) // OK
{
	...
	//if(lpTarget->Index != OBJECT_MONSTER && lpTarget->Map != MapCTC){
...
bool CSkillManager::SkillPartyHeal(int aIndex,int bIndex,CSkill* lpSkill) // OK
{
	...
	if(BATTLE_SURVIVOR_BSV(lpObj->Map) != 0)
	{
		return 0;
	}
...
void CSkillManager::CGMultiSkillAttackRecv(PMSG_MULTI_SKILL_ATTACK_RECV* lpMsg,int aIndex,bool type) // OK
{
	...
	//==== Antihack
	if (!gBProtect.CheckAnimationSkill(lpObj->Index, skill,0))
	{
		return;
	}
	...
	for(int n=0;n < lpMsg->count;n++)
	{
		...
		//============ Auto Skill Target
		if (!gBProtect.AntiAutoSkill(aIndex,bIndex))
		{
			return;
		}
...
void CSkillManager::CGSkillAttackRecv(PMSG_SKILL_ATTACK_RECV* lpMsg,int aIndex) // OK
{
	...
	if (BC_MAP_RANGE(lpObj->Map) || CA_MAP_RANGE(lpObj->Map) || CC_MAP_RANGE(lpObj->Map) || DS_MAP_RANGE(lpObj->Map) || IT_MAP_RANGE(lpObj->Map) || BATTLE_SURVIVOR_BSV(lpObj->Map))
	{
	...
	if (!gBProtect.CheckAnimationSkill(lpObj->Index, skill, 1))
	{
		return;
	}
	#if(HACKSPEEDCHECK == 1)
		if ( !gHackSkillCheck.SpeedHackCheck(lpObj->Index,2,skill) ) //check Hack Speed
		{
			return;
		}
	#endif
...
void CSkillManager::CGDurationSkillAttackRecv(PMSG_DURATION_SKILL_ATTACK_RECV* lpMsg,int aIndex) // OK
{
	...
	#if(HACKSPEEDCHECK == 1)
		if ( !gHackSkillCheck.SpeedHackCheck(lpObj->Index,3,skill) ) //check Hack Speed
		{
			return;
		}
	#endif

	if (!gBProtect.CheckAnimationSkill(lpObj->Index, skill, 2))
	{
		return;
	}
	...
	{
		//gLog.Output(LOG_HACK,"[SENDPACKAGESKILL][%s][%s] - (Skill: %d)",lpObj->Account,lpObj->Name,lpObj->Account,lpSkill->m_index);
		//LogAdd(LOG_RED,"[SENDPACKAGESKILL][%s][%s] - (Skill: %d)",lpObj->Account,lpObj->Name,lpObj->Account,lpSkill->m_index);
		return;
	}
	...
	if (!gBProtect.CheckAnimationSkill(lpObj->Index, lpSkill->m_skill, 99))
	{
		//LogAdd(LOG_RED,"TEST HACK");
		return;
	}

	
	//if(
//   lpSkill->m_index ==  SKILL_MANA_SHIELD 
	//|| lpSkill->m_index ==  SKILL_CRESCENT_MOON_SLASH 
	//|| lpSkill->m_index ==  SKILL_SPIRAL_SLASH 
	//|| lpSkill->m_index ==  SKILL_MANA_RAYS 
	//|| lpSkill->m_index ==  SKILL_FIRE_BLAST 
	//|| lpSkill->m_index ==  SKILL_MANA_GLAIVE 
	//|| lpSkill->m_index ==  SKILL_STAR_FALL 
	//|| lpSkill->m_index ==  SKILL_CHARGE 
	//|| lpSkill->m_index ==  SKILL_HEAL 
	//|| lpSkill->m_index ==  SKILL_PARTY_HEAL 
	//|| lpSkill->m_index ==  SKILL_DEFENSE 
	//|| lpSkill->m_index ==  SKILL_GREATER_DEFENSE 
	//|| lpSkill->m_index ==  SKILL_GREATER_DAMAGE 
	//|| lpSkill->m_index ==  SKILL_GREATER_CRITICAL_DAMAGE 
	//|| lpSkill->m_index ==  SKILL_SHIELD_RECOVER 
	//|| lpSkill->m_index ==  SKILL_DAMAGE_REFLECT 
	//)
	//{
	//	gLog.Output(LOG_HACK,"[SENDPACKAGESKILL][%s][%s] - (Skill: %d)",lpObj->Account,lpObj->Name,lpObj->Account,lpSkill->m_index);
	//	LogAdd(LOG_RED,"[SENDPACKAGESKILL][%s][%s] - (Skill: %d)",lpObj->Account,lpObj->Name,lpObj->Account,lpSkill->m_index);
...
void CSkillManager::CGRageFighterSkillAttackRecv(PMSG_RAGE_FIGHTER_SKILL_ATTACK_RECV* lpMsg,int aIndex) // OK
{
	...
	if(gHackSkillCheck.CheckSpeedHack(lpObj,lpSkill->m_skill) == 1)
	{
		return;
	}
	#if(HACKSPEEDCHECK == 1)
		if ( !gHackSkillCheck.SpeedHackCheck(lpObj->Index,4,lpSkill->m_skill) ) //check Hack Speed
		{
			return;
		}
	#endif	
	...
	if (!gBProtect.CheckAnimationSkill(lpObj->Index, lpSkill->m_skill, 3))
	{
		return;
	}
...
void CSkillManager::CGSkillDarkSideRecv(PMSG_SKILL_DARK_SIDE_RECV* lpMsg,int aIndex) // OK
{
	...
	#if(HACKSPEEDCHECK == 1)
		if ( !gHackSkillCheck.SpeedHackCheck(lpObj->Index,5,lpSkill->m_skill) ) //check Hack Speed
		{
			return;
		}
	#endif

	if (!gBProtect.CheckAnimationSkill(lpObj->Index, lpSkill->m_skill, 4))
	{
		return;
	}
