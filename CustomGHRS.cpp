...
void CCustomGHRS::Load(char* path) {
	this->GHRS_WEEK[1] = GetPrivateProfileInt("WEEK", "Monday", 10, path);
	this->GHRS_WEEK[2] = GetPrivateProfileInt("WEEK", "Tuesday", 10, path);
	this->GHRS_WEEK[3] = GetPrivateProfileInt("WEEK", "Wednesday", 10, path);
	this->GHRS_WEEK[4] = GetPrivateProfileInt("WEEK", "Thursday", 10, path);
	this->GHRS_WEEK[5] = GetPrivateProfileInt("WEEK", "Friday", 10, path);
	this->GHRS_WEEK[6] = GetPrivateProfileInt("WEEK", "Saturday", 15, path);
	this->GHRS_WEEK[0] = GetPrivateProfileInt("WEEK", "Sunday", 15, path);
...
void CCustomGHRS::GDCustomGHRSRecv(SDHP_CUSTOM_GHRS_RECV* lpMsg) {
	...
	this->Day = lpMsg->time;
	LogAdd(LOG_BLUE, "[%02d:%02d:%04d][GHRS] Cap nhat GHRS hom nay: %d (Top 1 00h duoc reset +%d)", ServerT->tm_mday, ServerT->tm_mon + 1, ServerT->tm_year + 1900, this->maxResets, this->GHRS_WEEK[ServerT->tm_wday]);
}
