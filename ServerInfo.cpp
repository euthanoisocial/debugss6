...
void CServerInfo::SendServerInfo(int aIndex)
{
	PMSG_SERVERINFO_SEND pMsg;
	pMsg.header.set(0xFB, 0x07, sizeof(pMsg));

	for (int i = 0; i<MAX_ACCOUNT_LEVEL; i++)
	{
		pMsg.CustomCharInterface[i] = this->m_CustomCharInterface[i];
		pMsg.MaxStatPoint[i] = this->m_MaxStatPoint[i];
	}
	pMsg.CustomWarehouseAnimated = this->m_CustomWarehouseAnimated;
	pMsg.Strength = gDefaultClassInfo.m_DefaultClassInfo[gObj[aIndex].Class].Strength;
	pMsg.Dexterity = gDefaultClassInfo.m_DefaultClassInfo[gObj[aIndex].Class].Dexterity;
	pMsg.Vitality = gDefaultClassInfo.m_DefaultClassInfo[gObj[aIndex].Class].Vitality;
	pMsg.Energy = gDefaultClassInfo.m_DefaultClassInfo[gObj[aIndex].Class].Energy;
	pMsg.Leadership = gDefaultClassInfo.m_DefaultClassInfo[gObj[aIndex].Class].Leadership;
	pMsg.PartyTeleportSwitch = this->m_PartyTeleportSwitch;
	pMsg.PartyTeleportMoney = this->m_PartyTeleportMoney;
...
void CServerInfo::ReadCommonInfo() // OK
{
	...
	if (gServerInfo.m_ServerLanguage == 0)	
	{	
		gMessage.Load(gPath.GetFullPath("Util\\Language\\Message_eng.txt"));
	return;
	}

	if (gServerInfo.m_ServerLanguage == 1)	
	{	
		gMessage.Load(gPath.GetFullPath("Util\\Language\\Message_spn.txt"));
	return;
	}

	if (gServerInfo.m_ServerLanguage == 2)	
	{	
		gMessage.Load(gPath.GetFullPath("Util\\Language\\Message_por.txt"));
	return;
	}
	//--
	gMiniMap.Load(gPath.GetFullPath("MiniMap.txt"));

	gPKSystem.Load(gPath.GetFullPath("PKSystem.xml"));
...
////////////////////
...
#if(QUAEVENT == 1)
	gQuaTop1.Load(gPath.GetFullPath("Custom\\QuaEvent\\QuaTop1.txt"));
	gQuaTop1.Load();
	gQuaTop2.Load(gPath.GetFullPath("Custom\\QuaEvent\\QuaTop2.txt"));
	gQuaTop2.Load();
	gQuaTop3.Load(gPath.GetFullPath("Custom\\QuaEvent\\QuaTop3.txt"));
	gQuaTop3.Load();
#endif
...
#if(OFFLINE_MODE_NEW)
	gOfflineMode.LoadSkillsInfo(gPath.GetFullPath("Custom\\OfflineMode\\OfflineModeBuffs.xml"));
	...
	gTiecRuouEvent.ReadCustomTiecRuouInfo("GameServerInfo",".\\Data\\GameServerInfo - Custom.dat");

	gTiecRuouEvent.Load(gPath.GetFullPath("Custom\\TiecRuouGuild.dat"));
	...
	//gCustomJewel.Load(gPath.GetFullPath("Custom\\CustomJewel.xml"));
	...
	//-- Custom Monster Reward
	gMonsterReward.Load(gPath.GetFullPath("Custom\\MonsterReward.txt"));
	...
	gHuyHieu.Load(gPath.GetFullPath("Custom\\CuongHoa\\CuongHoaSetItem.txt"));

	gCuongHoaWing.Load(gPath.GetFullPath("Custom\\CuongHoa\\CuongHoaWing.txt"));

	gVuKhiRong.Load(gPath.GetFullPath("Custom\\CuongHoa\\CuongHoaVuKhiRong.txt"));
...
#if(MONSTERITEM)
	gMonsterCall.Load(gPath.GetFullPath("Custom\\MonsterCall.txt"));
#endif
	//----------
#if(THUE_VIP_NEW)
	gThueVip.LOAD_TXT();
	...
	//----------
#if(DANH_HIEU_NEW)
	gDanhHieu.Load(gPath.GetFullPath("Custom\\DanhHieu\\DanhHieuDamage.txt"));
	gDanhHieu.Init();
	gDanhHieu.Load();
#endif
	//----------
#if(QUAN_HAM_NEW)
	gQuanHam.Load(gPath.GetFullPath("Custom\\DanhHieu\\QuanHamDamage.txt"));
	gQuanHam.Init();
	gQuanHam.Load();
#endif
	//----------
	...
	gSachThuocTinh.Load_Text();
#endif
	//----------
#if(RINGPEDAN_NEW)
	gRingPedan.Load_Text();
#endif
	//----------
#if (GHRS_TOP1_NEW)
	gCustomGHRS.Load(gPath.GetFullPath("Custom\\CustomGHRS.txt"));
#endif
	//=========JewelBank
//tue add bau cua 
#if BAUCUA
	gCustomBauCua.Load();
	LogAdd(LOG_BLUE, "[ServerInfo] load Bầu Cua thành công");
#endif
//------
...
void CServerInfo::ReadEventInfo() // OK
{
	...
	gCTCMini.Load(gPath.GetFullPath("Event\\CTCMini\\Config.xml"));
	#if(CHIEN_TRUONG_CO)
	gChienTruongCo.LoadData(gPath.GetFullPath("Event\\ChienTruongCo.xml"));
#endif
	gBsVEvent.Load(gPath.GetFullPath("Event\\BsVEvent.dat"));
	//
	// Leo thap
	gLeoThap.Load(gPath.GetFullPath("Event\\LeoThap.dat"));
	//
...
#if(BOSS_GUILD == 1)
	gBossGuild.Load(gPath.GetFullPath("Event\\BossGuild\\Config.xml"));
	gBossGuild.Init();
#endif
...
void CServerInfo::ReadHackInfo() // OK
{
	...
	gHackXDameCheck.Load(gPath.GetFullPath("Hack\\HackXDameCheck.txt"));
	...
	gBProtect.LoadConfig(gPath.GetFullPath("Hack\\SpeedCheck.xml"));
...
void CServerInfo::ReadBuffMuSolar() // OK
{
	gEffectManager.Load(gPath.GetFullPath("BuffMuSolar.txt"));

	LogAdd(LOG_RED,"[ServerInfo] BuffMuSolar Loader Thanh Cong");
}
...
#if(BOTSTATUE == 1)
	ObjBotStatue.Read(gPath.GetFullPath("Custom\\BotSystem\\BotStatue.txt"));
	ObjBotStatue.Init();
#endif
...
void CServerInfo::ReadStartupInfo(char* section,char* path) // OK
{
	...
	this->m_WriteGrandResetLog = GetPrivateProfileInt(section, "WriteGrandResetLog", 0, path);
}
...
void CServerInfo::ReadCharacterInfo(char* section,char* path) // OK
{
	...
	//this->m_MoGioiHanDameQuai = GetPrivateProfileInt(section,"MoGioiHanDameQuai",0,path);
...
void CServerInfo::ReadCommandInfo(char* section,char* path) // OK
{
	...
	this->CommandResetPoint_WCoinC = GetPrivateProfileInt(section, "CommandResetPoint_WCoinC", 0, path);

	this->CommandResetMaster_WCoinC = GetPrivateProfileInt(section, "CommandResetMaster_WCoinC", 0, path);

	this->CommandChange_WCoinC = GetPrivateProfileInt(section, "CommandChange_WCoinC", 0, path);

#if(RESETLIFE_NEW)
	this->CommandRelife_Check_Level = GetPrivateProfileInt(section, "CommandRelife_Check_Level", 0, path);

	this->CommandRelife_Check_Reset = GetPrivateProfileInt(section, "CommandRelife_Check_Reset", 0, path);

	this->CommandRelife_Level = GetPrivateProfileInt(section, "CommandRelife_Level", 0, path);

	this->CommandRelife_Reset = GetPrivateProfileInt(section, "CommandRelife_Reset", 0, path);

	this->CommandRelife_ResetLife = GetPrivateProfileInt(section, "CommandRelife_ResetLife", 0, path);

	this->CommandRelife_Point = GetPrivateProfileInt(section, "CommandRelife_Point", 0, path);

	this->CommandRelife_Max = GetPrivateProfileInt(section, "CommandRelife_Max", 0, path);

	this->CommandRelife_MaxClient = GetPrivateProfileInt(section, "CommandRelife_MaxClient", 0, path);
...
void CServerInfo::ReadCommonInfo(char* section,char* path) // OK
{
	...
	//Multi Language System
	this->m_ServerLanguage = GetPrivateProfileInt(section,"ServerLanguage",0,path);
	...
	this->m_SocketHarmony = GetPrivateProfileInt(section, "AllowSocketHarmony", 0, path);
	...
	this->m_MonsterDamageRate = GetPrivateProfileInt(section, "MonsterDamageRate", 0, path);
	...
	/// config buff elf
	this->m_SatThuongVatLy = GetPrivateProfileInt(section,"SatThuongVatLy",0,path);
	this->m_SatThuongPhep = GetPrivateProfileInt(section,"SatThuongPhep;",0,path);
	this->m_LoiNguyen = GetPrivateProfileInt(section,"LoiNguyen",0,path);
	this->m_PhongThu = GetPrivateProfileInt(section,"PhongThu",0,path);
	this->m_CritDamageDL = GetPrivateProfileInt(section,"CritDamageDL",0,path);
	this->m_ManaShiled = GetPrivateProfileInt(section,"ManaShiled",0,path);
	this->m_GongMauDK = GetPrivateProfileInt(section,"GongMauDK",0,path);
	this->m_PhanDamageSum = GetPrivateProfileInt(section,"PhanDamageSum",0,path);
	////

	this->m_CungGuildPK = GetPrivateProfileInt(section,"CungGuildPK", 0, path);
	...
	this->m_PartyDivideZen = GetPrivateProfileInt(section, "PartyDivideZen", 0, path);

	this->m_CreateCoinWhenZenIsMax[0] = GetPrivateProfileInt(section, "CreateCoinWhenZenIsMax_AL0", 0, path);

	this->m_CreateCoinWhenZenIsMax[1] = GetPrivateProfileInt(section, "CreateCoinWhenZenIsMax_AL1", 0, path);

	this->m_CreateCoinWhenZenIsMax[2] = GetPrivateProfileInt(section, "CreateCoinWhenZenIsMax_AL2", 0, path);

	this->m_CreateCoinWhenZenIsMax[3] = GetPrivateProfileInt(section, "CreateCoinWhenZenIsMax_AL3", 0, path);

	this->m_CreateCoinIndex = GetPrivateProfileInt(section, "CreateCoinIndex", 0, path);

	this->m_CreateCoinPrice = GetPrivateProfileInt(section, "CreateCoinPrice", 0, path);
	...
	this->m_GensExitTime[0] = GetPrivateProfileInt(section, "GensSystemExitTime_AL0", 0, path);

	this->m_GensExitTime[1] = GetPrivateProfileInt(section, "GensSystemExitTime_AL1", 0, path);

	this->m_GensExitTime[2] = GetPrivateProfileInt(section, "GensSystemExitTime_AL2", 0, path);

	this->m_GensExitTime[3] = GetPrivateProfileInt(section, "GensSystemExitTime_AL3", 0, path);

	this->m_GensSystemSelfGensDamage = GetPrivateProfileInt(section, "GensSystemSelfGensDamage", 0, path);
	...
	this->m_PartyTeleportSwitch = GetPrivateProfileInt(section, "PartyTeleportSwitch", 0, path);

	this->m_PartyTeleportMoney = GetPrivateProfileInt(section, "PartyTeleportMoney", 0, path);
	...
	this->m_ThoiGianThoatGame = GetPrivateProfileInt(section, "ThoiGianThoatGame", 6, path);
}
...
void CServerInfo::ReadCustomInfo(char* section,char* path) // OK
{
	...
	this->m_CustomCharInterface[0] = GetPrivateProfileInt(section, "CustomCharInterface_AL0", 0, path);

	this->m_CustomCharInterface[1] = GetPrivateProfileInt(section, "CustomCharInterface_AL1", 0, path);

	this->m_CustomCharInterface[2] = GetPrivateProfileInt(section, "CustomCharInterface_AL2", 0, path);

	this->m_CustomCharInterface[3] = GetPrivateProfileInt(section, "CustomCharInterface_AL3", 0, path);
	...
	this->m_CustomWarehouseAnimated = GetPrivateProfileInt(section, "CustomWarehouseAnimated", 0, path);

		this->m_CheckOptionsDrop = GetPrivateProfileInt(section,"CheckOptionsDrop",0,path);

	this->m_OptionPecent = GetPrivateProfileInt(section,"OptionPecent",0,path);
	this->m_SkillPecent = GetPrivateProfileInt(section,"SkillPecent",0,path);																																																			  																								
	this->m_LuckPecent = GetPrivateProfileInt(section,"LuckPecent",0,path);
	char ExlOption[128]={0};
	char VKExlOption[128]={0};
	char PhuKienExlOption[128]={0};  
    for(int n=0; n < 6; n++) 
	{
		wsprintf(ExlOption,"ExlOption_%02d", n);
		this->NewItemDrop[n] = GetPrivateProfileInt(section,ExlOption,0,path); 

 		wsprintf(VKExlOption,"VKExlOption_%02d", n);
		this->NewVKDrop[n] = GetPrivateProfileInt(section,VKExlOption,0,path); 

  		wsprintf(PhuKienExlOption,"PhuKienExlOption_%02d", n);
		this->NewPhuKienDrop[n] = GetPrivateProfileInt(section,PhuKienExlOption,0,path); 
	}
...
void CServerInfo::ReadEventInfo(char* section,char* path) // OK
{
	...
	//--bloodvip
	this->m_BCVIP = GetPrivateProfileInt(section, "BloodVIPOnOff", 0, path);
	this->m_DiemBlood = GetPrivateProfileInt(section, "ScoreBloodVIP", 0, path);
	this->m_DVVIP = GetPrivateProfileInt(section, "DevilVIPOnOff", 0, path);
	this->m_DiemDevil = GetPrivateProfileInt(section, "ScoreDevilVIP", 0, path);
	//--
	...
	this->m_LeoThapEvent = GetPrivateProfileInt(section,"LeoThapEvent",0,path);
	
	this->m_LeoThapMap = GetPrivateProfileInt(section,"LeoThapMap",0,path);
	...
	this->m_BsVEventSwitch					= GetPrivateProfileInt(section,"EventBsVSwitch",0,path);
	this->m_BsVEventNPC						= GetPrivateProfileInt(section,"EventBsVNpc",0,path);
	this->m_BsVEventNPCMap					= GetPrivateProfileInt(section,"EventBsVNpcMap",0,path);
	this->m_BsVEventNPCX					= GetPrivateProfileInt(section,"EventBsVNpcX",0,path);
	this->m_BsVEventNPCY					= GetPrivateProfileInt(section,"EventBsVNpcY",0,path);
	this->m_BsVEventMinUsers				= GetPrivateProfileInt(section,"EventBsVMinUsers",2,path);
	this->m_BsVEventMaxUsers				= GetPrivateProfileInt(section,"EventBsVMaxUsers",20,path);
...
void CServerInfo::ReadSkillInfo(char* section,char* path) // OK
{
	...
	this->PhongThu12 = GetPrivateProfileInt("CONFIG", "PhongThu12", 0, "./HuyHieuCuongHoa.ini");
	this->PhongThu13 = GetPrivateProfileInt("CONFIG", "PhongThu13", 0, "./HuyHieuCuongHoa.ini");
	this->PhongThu14 = GetPrivateProfileInt("CONFIG", "PhongThu14", 0, "./HuyHieuCuongHoa.ini");
	this->PhongThu15 = GetPrivateProfileInt("CONFIG", "PhongThu15", 0, "./HuyHieuCuongHoa.ini");


	this->Mau11 = GetPrivateProfileInt("CONFIG", "Mau11", 0, "./HuyHieuCuongHoa.ini");
	this->Mau12 = GetPrivateProfileInt("CONFIG", "Mau12", 0, "./HuyHieuCuongHoa.ini");
	this->Mau13 = GetPrivateProfileInt("CONFIG", "Mau13", 0, "./HuyHieuCuongHoa.ini");
	this->Mau14 = GetPrivateProfileInt("CONFIG", "Mau14", 0, "./HuyHieuCuongHoa.ini");
	this->Mau15 = GetPrivateProfileInt("CONFIG", "Mau15", 0, "./HuyHieuCuongHoa.ini");

	this->BQPN1 = GetPrivateProfileInt("CONFIG", "BQPN1", 0, "./HuyHieuCuongHoa.ini");
	this->BQPN2 = GetPrivateProfileInt("CONFIG", "BQPN2", 0, "./HuyHieuCuongHoa.ini");
	this->BQPN3 = GetPrivateProfileInt("CONFIG", "BQPN3", 0, "./HuyHieuCuongHoa.ini");

	this->X2Dame = GetPrivateProfileInt("CONFIG", "X2Dame", 0, "./HuyHieuCuongHoa.ini");
#endif
}
...
#if(DEV_DAMAGE_TABLE)
void CServerInfo::ResetDamageTable()
{
	if(gServerInfo.EnableTable)
	{
		return;
	}

	for(int aIndex = OBJ_STARTUSERINDEX; aIndex < MAX_OBJECT; aIndex++)
	{
		LPOBJ lpUser = &gObj[aIndex];
		...
		gServerInfo.GCDamageTable(aIndex);
	}
}

void CServerInfo::GCDamageTable(int aIndex)
{
	LPOBJ lpUser = &gObj[aIndex];
