...
void CCustomArena::ReadCustomArenaInfo(char* section,char* path) // OK
{
	...
	GetPrivateProfileString(section, "CustomArenaText20", "", this->m_CustomArenaText20, sizeof(this->m_CustomArenaText20), path);
	this->m_CustomArenaWCoinC = GetPrivateProfileInt(section, "CustomArenaWCoinC", 0, path);
}
	...
	for(int n=0;n < MAX_CUSTOM_ARENA_USER;n++)
	{
		if(OBJECT_RANGE(lpInfo->User[n].Index) != 0)
		{
			if(lpInfo->User[n].Rank == 1)
			{
				...
				gNotice.GCNoticeSendToAll(0,0,0,0,4,0,this->m_CustomArenaText6,lpInfo->RuleInfo.Name,gObj[lpInfo->User[n].Index].Name);
				...
				// Dev Add WCoin Custom Arena
				gCashShop.GDCashShopAddPointSaveSend(lpInfo->User[n].Index, 0, this->m_CustomArenaWCoinC, 0, 0, 0);
				gNotice.GCNoticeSend(lpInfo->User[n].Index, 1, 0, 0, 0, 0, 0, this->m_CustomArenaText20, lpInfo->RuleInfo.Name, this->m_CustomArenaWCoinC);
				gCashShop.CGCashShopPointRecv(lpInfo->User[n].Index);
