...
void CParty::CGPartyRequestRecv(PMSG_PARTY_REQUEST_RECV* lpMsg, int aIndex) // OK
{
	...
	//--FinPartyLevel
	if(BATTLE_SURVIVOR_BSV(lpTarget->Map) != 0){
		return;
	}
	...
	//if (g_Survivor.IsInSurvivorArea(&gObj[bIndex]))
	//{
	//	return;
	//}
...
void CParty::CGNewLeader(int aIndex, GC_PARTYNEWLEADER* aRecv){
	...
	LogAdd(LOG_RED, "CGNewLeader %d", this->m_PartyInfo[index].Index[aRecv->id]);
	if (this->m_PartyInfo[index].Index[aRecv->id] == -1 || aRecv->id > MAX_PARTY_USER){
		return;
	}
	this->m_PartyInfo[index].Index[0] = this->m_PartyInfo[index].Index[aRecv->id];
	this->m_PartyInfo[index].Index[aRecv->id] = aIndex;
	this->GCPartyListSend(index);
	LogAdd(LOG_RED, "CGNewLeader %d | %d | %d", aRecv->id, this->m_PartyInfo[index].Index[0], aIndex);
}
#endif